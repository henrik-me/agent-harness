...
This project's `repo-short` is **`ah`** (defined in
...
|--------------------|---------------|------------------|
| `<repo>_copilot<N>` | `-c<N>` | `yoga-ah-c2` |
| `<repo><N>` (trailing digits) | `-c<N>` | `yoga-ah-c3` |
| anything else | _(omitted)_ | `yoga-ah` |
...
```
HARNESS_AGENT_AH_MACHINE=<override>
```
...
env-var-name:   HARNESS_AGENT_AH_MACHINE
...
| Column | Description |
|--------|-------------|
| `Agent ID` | Derived agent ID (e.g. `yoga-ah`) |
