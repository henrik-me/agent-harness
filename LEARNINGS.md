...
- **Static audit:** `git grep -nE '\b(checkout|switch|restore|reset|clean|stash|worktree)\b' -- lib bin scripts` finds only read-only `git describe --tags --exact-match` + `git rev-parse` (in `lib/sync.mjs`) and unrelated JS `switch` statements. The same grep at tag `v0.5.1` and `git log --all -G` over ALL history confirm **no `git checkout <tag>` has ever existed in the harness source** (two false-positive hits only: a comment + a JS `switch`). No git hooks (only `*.sample`); no npm lifecycle scripts (`package.json` `scripts` = `{"test": "node --test"}`).
