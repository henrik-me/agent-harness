...
**Problem:** The `Model` column in `## Review log` rows MUST be the bare reviewer-model identifier (e.g. `gpt-5.5`, `claude-sonnet-4.6`) — NOT decorated with parenthetical annotations like `gpt-5.5 (reviewer)` or `gpt-5.5 (R2)`. The `scripts/checks/check-review-log-evidence.mjs` PR-side gate parser normalises and compares against the primary reviewer model (with fallback rationale per REVIEWS.md §2.2); any decoration like `gpt-5.5 (reviewer)` fails the normalised comparison. The fact is present in agent memory but not documented in `LEARNINGS.md`, `REVIEWS.md`, or as schema validation — so it is rediscovered each time a new orchestrator drafts a Review log row (~quarterly cadence given LLM rotation).

**Finding:** **Lock the bare-model-id rule into REVIEWS.md § 2.8 (PR body requirements, which contains the Review log schema) and add a regression test alongside `tests/cs51-review-gates-logic.test.mjs` (which covers `scripts/checks/check-review-log-evidence.mjs`) that fails when `Model = 'gpt-5.5 (reviewer)'`.** The fix is purely documentation + test:

1. REVIEWS.md § 2.8 (under the Review log schema description) Model column description: add "MUST be the bare reviewer-model identifier (e.g. `gpt-5.5`); decorations like `gpt-5.5 (reviewer)`, `gpt-5.5 (R2)`, `gpt-5.5 (PvI)` are not permitted — use the `actor` column (e.g. `rubber-duck (PvI R2)`) for round/role annotations instead."
2. Add a fixture test (extend `tests/cs51-review-gates-logic.test.mjs` or co-locate in a new `tests/check-review-log-evidence.test.mjs`): a Review log row with decorated `Model` column should produce a clear error from `scripts/checks/check-review-log-evidence.mjs` saying "decorated model identifier; use bare `<model>` and put annotations in the actor column".
