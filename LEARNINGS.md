...
**Disposition:** Applied in CS54. Codified in `OPERATIONS.md § Cross-repo procedures` as new `### Cross-repo pin-bump PR body checklist (CS54)` H3, with a short pointer in `template/managed/.github/copilot-instructions.md` Rule 6. The doctrine states: consumer-repo PRs (typically pin bumps) MUST include `## Summary` / `## Changes` / `## Testing` / `## Model audit` (with `Implementer agent` and `Reviewer agent` rows, mandatory since v0.6.0 strict-flip) / `## Review log` (6-column, bare reviewer-model id per LRN-136 → REVIEWS.md § 2.8) / plan link at PR-open time. Pre-open self-check requires draft body grep before `gh pr create`. CS54a/CS54b in LRN-134 original disposition are subsumed (CS54a doctrine landed here; CS54b consumer template refresh remains an open follow-up — file as separate planned CS when prioritised). Cross-references: LRN-135 (narrow re-attest after body push); LRN-136 (Review log Model bare-id rule).
...
1. `scripts/checks/check-review-log-evidence.mjs`: decoration-detection check inserted BEFORE `reviewerModelApproved()`. Emits explicit error pointing to the bare identifier and to `REVIEWS.md § 2.8`. Closes the silent-pass path where decorated cells (e.g. `gpt-5.5 (R2)`) normalised to `gpt-5.5-r2`, failed the primary check, and the fallback-rationale path approved them. Did NOT touch `check-independence-invariant.mjs` (preserves existing normalisation behaviour).
...
3. `tests/cs51-review-gates-logic.test.mjs`: new regression case covering (a) decorated model + populated fallback rationale must be rejected with the decoration message (the historically-silent path), (b) decorated model with no fallback must be rejected with the decoration message (not the unrelated fallback-missing message), (c) bare reviewer model still passes when the body is well-formed. 8/8 tests pass.
