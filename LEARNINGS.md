...
3. `tests/cs51-review-gates-logic.test.mjs`: new regression case "review-log-evidence rejects decorated reviewer model identifiers (LRN-136)" with multiple sub-cases covering decorated model + populated fallback rationale (the historically-silent path), decorated model with no fallback, bare reviewer model positive control, `(reviewer)` annotation, uniform enforcement on a non-passing (`Needs-Fix`) row, and non-parenthesized decorations (`gpt-5.5 R2`, `gpt-5.5 - R2`, `gpt-5.5/R2`) per the bare-id rule `/^[A-Za-z0-9._-]+$/`. Full suite passes 1021/0/1; targeted cs51 + cs48 + operations-reviewer-preamble suites pass 13/13.
