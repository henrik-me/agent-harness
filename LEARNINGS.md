...
**Evidence:** `scripts/check-workflow-pins.mjs:283` (`totalErrors++`) vs `:146` (`const errors = []`), `:153-154` (`logError` pushes to `errors`), `:242` (`let totalPinsChecked = 0` — the only sibling counter; no `totalErrors` declared). Surfaced independently by both a CS-plan draft sub-agent and the orchestrator during 2026-07-05 issue triage; verified by grep (only line 283 references `totalErrors`).

**Disposition:** Open — CS112 (harness-owned managed-workflow action bumps, whose deliverables exercise the `workflow-pins` linter) is the natural fix vehicle; repair the `:283` branch to use `logError()` as a tightly-coupled bug. claim_area: linters.
...
source_cs: CS90a
status: open
tags: [copilot, review, false-positive, hallucination, verification, action-pins, chase]
claim_area: review-gates
```

**Problem:** Copilot's PR review can assert confident but **factually false** claims that, if "fixed", would corrupt correct code. On CS90a PR #516 (HEAD `7655f75`) it flagged the `actions/checkout@de0fac2e...` SHA pin in two threads as "corresponds to v4.1.1 (not v6.0.2)" — a hallucination that, actioned, would have rewritten a correct pin comment to the wrong tag.

**Finding:** Treat any Copilot finding that makes a **checkable factual claim** (SHA↔tag, version numbers, API shapes, file/line contents) as UNVERIFIED until confirmed against the authoritative source. Here `gh api repos/actions/checkout/git/ref/tags/v6.0.2` resolves to exactly `de0fac2e4500dabe0009e67214ff5f5447ce83dd` (v4.1.1 is a different SHA, `b4ffde65...`), and 8+ of this repo's own workflows pin the same SHA `# v6.0.2` — so the comment was correct. Resolve a disproven finding as a **false positive with the evidence in the reply**; do not edit. Copilot is also **non-deterministic across re-engagements on byte-identical content** (R7 → 0 findings; R10 → 3 new incl. the hallucination on an identical diff), so a clean prior Copilot pass does not guarantee the next is clean.

**Evidence:** PR #516 @ `7655f75`: Copilot COMMENTED opened 3 threads (2× SHA-pin "v4.1.1" + 1× `git checkout` ref-vs-pathspec). Tag resolution disproved the SHA claims → resolved 2 as false positives with cited evidence; the 3rd was a genuine improvement (`git checkout --detach` fail-closed) and was fixed in `ea4c609`. Re-engaging Copilot at `ea4c609` → 0 findings. Full content-review span: 11 gpt-5.5 rubber-duck rounds + 6 Copilot re-engages across 2 rebases onto the CS109a base.

**Disposition:** Open — orchestrator guidance: verify factual Copilot claims against upstream/repo before acting; **bound the chase loop** — fix genuine findings, resolve disproven/false-positive ones with cited evidence, then merge on the next clean Copilot pass rather than re-engaging indefinitely. Complements LRN-125 (Copilot chase), LRN-200 (engage Copilot last), LRN-217 (async gate rerun).
...
source_cs: CS108
status: open
tags: [file-class, reclassification, init, seeded-config, sync]
claim_area: sync
```

**Problem:** A managed→composed (or any file-class) reclassification must be applied in TWO independent config sources: the self-host `harness.config.json` AND `template/seeded/harness.config.json` (the fresh-`init` defaults). CS108 initially edited only the self-host config; the fresh-`init` tests (`cs09-init`, plus init-materialization in CS15d/CS15e) then failed because a freshly-`init`-ed consumer would still classify the three docs as `managed`.

**Finding:** The two configs are separate sources of truth for the same classification decision; a reclassification that updates one but not the other ships inconsistent behavior (self-host composed, fresh consumers managed) and reddens the init test suite.

**Evidence:** CS108 moved `TRACKING.md`/`RETROSPECTIVES.md`/`READMEGUIDE.md` in both `harness.config.json` and `template/seeded/harness.config.json` (`managed.files` → `composed.files` + overrides; seeded `managed.files` became `[]`); `tests/cs09-init.test.mjs` required updated expectations. The implementer surfaced this as an explicit decision/escalation.

**Disposition:** Open — future file-class reclassification CSs must update `template/seeded/harness.config.json` in lockstep with `harness.config.json` (and refresh init tests). Candidate: a lint/test asserting the two configs agree on shared file classifications.
...
source_cs: CS90
status: open
tags: [harness-claim, clickstop, model-audit, tasks, lint]
claim_area: claim
```

**Problem:** `harness claim <CS> --apply` performs ONLY the `git mv` (planned→active) + inserts the WORKBOARD Active Work row. It does NOT flip the CS file's `**Status:**` planned→active, fill `**Owner:**`/`**Branch:**`/`**Started:**`, add the `## Model audit` section, or replace the `## Tasks` placeholder — so the just-claimed `active/` file fails `harness lint`.

**Finding:** The claim verb is intentionally minimal (it never commits). But the `active/` lifecycle imposes invariants the verb leaves unmet: `check-clickstop` requires `**Status:** active` + close-out docs/learnings task rows + (for distributed-surface `## Deliverables`) a CHANGELOG-touch row, and `check-clickstop-implementer-not-reviewer` requires a `## Model audit` with non-overlapping `Implementer models`/`Reviewer model`. All must be hand-populated by the orchestrator before the claim-PR lint passes.

**Evidence:** CS90 — after `claim CS90 --apply` the file still read `**Status:** planned` / `**Owner:** —` / the `(populated at claim time)` Tasks placeholder; `harness lint` failed `check-clickstop` (DIR_STATUS active + close-out + CHANGELOG-touch) and `check-clickstop-implementer-not-reviewer` ("## Model audit missing required model row(s)"). Fixed by hand-editing the header + adding `## Model audit` (implementer `claude-opus-4.8` ≠ reviewer `gpt-5.5`) + the `## Tasks` rows.

**Disposition:** Open — orchestrator guidance: immediately after `claim --apply`, hand-populate Status→active, Owner/Branch/Started, `## Model audit`, and `## Tasks` (deliverable + close-out + CHANGELOG-touch rows) before running the claim-PR `harness lint`. Candidate follow-up: `harness claim --apply` could scaffold these fields.
...
source_cs: CS90
status: open
tags: [read-only-gates, pr-evidence-lint, copilot, async, action-required, rerun]
claim_area: review-gates
```

**Problem:** On a self-host CONTENT PR (non-workboard-only), `read-only-gates` (`pr-evidence-lint.yml`) runs on the push `pull_request` events BEFORE Copilot's async (~3–4 min) review lands, so A16/A5 fail; and the `pull_request_review`-triggered re-run (fired by Copilot's own bot review submission) concludes **`action_required`** (0 jobs executed — GitHub gates workflow runs triggered by a bot actor), so it never flips the required check green. The gate stays red at a HEAD whose content is actually fine.

**Finding:** The stuck-red `read-only-gates` is a timing/approval artifact, not a content failure. After Copilot's review is verified at the current HEAD, re-running the failed **`pull_request`**-event run — `gh run rerun <id> --failed` — re-queries the now-present Copilot review and passes A5/A16 (the `pull_request_review` re-run cannot help — it is `action_required`). The `review-gates.yml` jobs (`copilot-review-attached` etc.) auto-pass since they run on `pull_request`, so only `read-only-gates` needs the manual re-run.

**Evidence:** CS90 PR #505 — at each HEAD the `read-only-gates` `pull_request` runs concluded `failure` (pre-Copilot); the `pull_request_review` run concluded `action_required` with 0 jobs. After `harness copilot-engage` verified the review at HEAD, `gh run rerun <pull_request-run-id> --failed` → conclusion `success`. Repeated at 989d1c5 and cbc18e7.

**Disposition:** Open — orchestrator guidance: after engaging Copilot and confirming its review at the current PR HEAD, `gh run rerun <failed read-only-gates pull_request run> --failed` to green the gate. Complements LRN-200 (engage Copilot last) and CS90c's own `pull_request_review`-trigger port (which fixes the review-gates half, not `read-only-gates`' `action_required`).
...
source_cs: CS108
status: open
tags: [harness-lint, text-encoding, git-ls-files, file-move, staging]
claim_area: linting
```

**Problem:** `harness lint`'s text-encoding check enumerates files via git (`git ls-files`), so a tracked-file MOVE (e.g. `template/managed/X.md` → `template/composed/X.md`) fails the linter with read-errors on the now-absent old paths until the moves are STAGED — a sub-agent bound by the no-`git add` preflight cannot make lint green on a rename.

**Finding:** The git-mode enumeration and on-disk reality diverge for an unstaged rename; the failure is a staging artifact, not a content defect. A filesystem-walk mode (`check-text-encoding --dir . --no-respect-gitignore`) confirms disk cleanliness independent of the index.

**Evidence:** CS108's sub-agent reported `harness lint` text-encoding failing on the 3 deleted-but-unstaged `template/managed/*.md` paths while an 800-file filesystem walk showed 0 violations; the failure cleared the moment the orchestrator staged the moves at commit time.

**Disposition:** Open — orchestrator guidance: stage managed→composed moves before the final `harness lint` gate; a sub-agent's lint-on-move failure is expected and resolves at the orchestrator commit. Candidate: the text-encoding linter could emit a notice that unstaged deletions are being skipped.
...
**Problem:** `.harness-lock.json`'s `template_prose_hash` for a composed file (`OPERATIONS.md`) was LATENTLY stale vs the actual root skeleton at HEAD — it escaped `sync --mode=check` only because the template and root were byte-identical, short-circuiting the divergence branch that would have consulted the hash. The staleness surfaces only when a composed-template PROSE edit forces the divergence path.

**Finding:** A stale `template_prose_hash` is invisible to `sync --mode=check` (which validates rendered CONTENT, not lock bookkeeping — cf. LRN-201) until a composed-prose edit lands: then the auto-adopt case (a) cannot match its own lock hash, and the root must be regenerated. CS108's OPERATIONS.md escape-valve edit hit exactly this and required a delete+regenerate of the root (which corrected the hash).
...
**Problem:** The **single-call** `harness review <pr>` flow (rubber-duck output piped to stdin, no `--copilot-only`) ALSO rewrites the PR body `## Model audit` **Reviewer agent** to the local orchestrator agent-id — on CS89 PR #500 it set `Reviewer agent = yoga-ah-c2`, equal to the `Implementer agent`, tripping the **A3** agent-identity gate ("Implementer agent and Reviewer agent are both yoga-ah-c2"). This contradicts LRN-197's speculation that "the canonical single-call `harness review <pr>` flow presumably avoids this" — it does not.

**Finding:** After ANY `harness review <pr>` (single-call OR `--copilot-only`), reset the `## Model audit` **Reviewer agent** back to a reviewer identity distinct from the implementer (e.g. `rubber-duck`) so A3 passes. Unlike the `--copilot-only` path (LRN-210), the single-call flow does NOT trip **A5**: `lib/review.mjs` records the local Go FIRST (`localGoAt = now`) then engages Copilot with `submittedAfter = localGoAt`, so Copilot's `submittedAt` (async, ~3 min later) correctly POSTDATES the local Go row. The single-call flow's Review-log actor is likewise the orchestrator id; aligning it to `rubber-duck` (matching prior rows) is harmless to A5 and consistent with LRN-210's "Review log rubber-duck-only" guidance.
...
source_cs: CS89
status: open
tags: [composed, marker-parser, duplication, dry, check-composed-blocks]
claim_area: composed
```

**Problem:** CS89 added the comment-safe `#`-marker form to the composed-block parser and had to mirror the SAME constants/logic into TWO hand-synced parsers — the runtime `lib/composed.mjs` (`checkMarkerLine` + `MARKER_HASH_*`) and the linter `scripts/check-composed-blocks.mjs` (its own `MARKER_HASH_*` + quick-skip) — and keep them byte-identical. Copilot's PR #500 review flagged a (transient) regex↔guard consistency gap in BOTH copies, illustrating the duplication hazard: a marker-syntax change must be applied identically in ≥2 places or the runtime parser and the lint gate silently diverge.

**Finding:** The two marker parsers are a DRY-candidate: `scripts/check-composed-blocks.mjs` could import the marker primitives from `lib/composed.mjs` the way the sibling `scripts/check-consumer-template-genericity.mjs` already imports `parseComposed`, so a single source of truth backs both runtime parsing and lint-time validation.

**Evidence:** CS89 touched `lib/composed.mjs:84-146` + `scripts/check-composed-blocks.mjs:118-160,211-224` in lockstep (`MARKER_HASH_RE`/`MARKER_HASH_CONTAINS` byte-identical); the R3 refinement + resolved Copilot threads on PR #500 both concerned keeping the two copies consistent. Precedent: `tests/cs11-self-host-config.test.mjs` already replaced a third hand-rolled marker extractor with `parseComposed` (CS89 sub-agent work).

**Disposition:** Open — candidate follow-up CS to DRY the composed marker parser (import from `lib/composed.mjs` into `scripts/check-composed-blocks.mjs`), eliminating the hand-sync burden.
