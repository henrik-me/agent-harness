...
**Problem:** Resuming a session after a mid-operation agent crash, every `git fetch origin` (and `fetch --prune`) aborted with `fatal: bad object refs/remotes/origin/cs62/close-out` / `error: ... did not send all necessary objects`. This blocked the Session-Start upstream sync, the staleness check, and the close-out rebase. The wording ("bad object", "necessary objects") points at the object database, misleadingly suggesting `git fsck` or a reclone, when the object store was actually intact.
