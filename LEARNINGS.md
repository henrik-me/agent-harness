...
**Evidence:** CS61 PR #250, 2026-06-06. Copilot rounds that each added one schema dimension: `additionalProperties:false` unknown-key fail-close (commit `a3bd65c`), `minLength:1` + non-whitespace `\S` pattern on `rubber_duck_model`/`fallback_model`/`copilot_reviewer_slug` (commits `5c2e24b` schema, `63d1496` pattern), `NOT_FOUND` message-path consistency (`8089c12`). The schema source is `schemas/harness.config.schema.json` `properties.reviews`. Cross-refs: LRN-145 (the § 2.6b reviewer checklist), LRN-039 (schema-is-source-of-truth), LRN-033 (fail-closed parsers).
...
source_cs: CS61
status: applied
tags: [git-merge, commit-trailer, co-authored-by, commit-trailers-gate, ci]
claim_area: orchestrator-loop
```
...
**Finding:** **Merge commits need the `Co-authored-by` trailer too** — `git commit --no-edit` (or any default merge message) on a `git merge` omits it, and `check-commit-trailers` (run by `harness lint` and the lint-aggregator tests against `HEAD`) treats the merge commit like any other. Fix: `git commit --amend` to add the trailer (or `git merge --no-ff -m "<msg>\n\nCo-authored-by: Copilot <223556219+Copilot@users.noreply.github.com>"`). Note the squash-merge of the PR discards the merge commit, but the trailer gate runs on branch commits *before* the squash, so the merge commit must still carry it.

**Evidence:** CS61, 2026-06-06. Merge commit `bc8e916` (no trailer) → 4 `node --test` failures in the lint-aggregator suites (`check-commit-trailers: fail`); amended to `844b92a` with the trailer → back to 1113 pass / 0 fail. The required trailer is defined in `.github/copilot-instructions.md` § Branch and commit conventions and enforced by `scripts/check-commit-trailers.mjs`.

**Disposition:** Applied — captured here as the standing operational rule; amend merge commits to carry the `Co-authored-by` trailer.
...
**Disposition update (2026-06-04, CS47 close-out — NO OFFENDER, environmental):** CS47 completed the bisection. **Conclusion: no `harness` CLI subcommand performs a HEAD-moving git operation — there is no in-source offender to fix.** The "shared helper" hypothesis above was disproved:
- **Static audit:** `git grep -nE '\b(checkout|switch|restore|reset|clean|stash|worktree)\b' -- lib bin scripts` matches only the words `clean`/`switch` inside comments, docstrings, and unrelated JS control-flow (e.g. `switch (status)`) — no git mutating verb; a manual audit confirms the only git calls in `lib/sync.mjs` are read-only `git describe --tags --exact-match` + `git rev-parse`. The same grep at tag `v0.5.1` and `git log --all -G` over ALL history confirm **no `git checkout <tag>` has ever existed in the harness source** (two false-positive hits only: a comment + a JS `switch`). No git hooks (only `*.sample`); no npm lifecycle scripts (`package.json` `scripts` = `{"test": "node --test"}`).
- **Dynamic bisection:** `tests/cs47-detached-head-bisect.test.mjs` enumerates every live `COMMAND_REGISTRY` subcommand and exercises the real code paths (NOT `--help`) — including the two live-repro subcommands `lint` and `plan-review-hash` — in both self-host and consumer modes. After every run, HEAD stays attached to its branch, `rev-parse HEAD == rev-parse <branch>`, the dirty tracked sentinel is preserved, and the per-run `GIT_TRACE` log records **no** HEAD/worktree-mutating git verb. 24/24 green.

**Therefore the LRN-124 detach was environmental** — not caused by the harness CLI source. Plausible environmental vectors (per C47-5, unproven, out of scope): a session shell shared with another process that ran `git checkout v0.5.1`, an IDE/editor git plugin, or a background tool operating on the same working tree. The operational mitigation stays in force unchanged ("commit after every multi-file edit batch, before any harness/gh command"), and `tests/cs47-detached-head-bisect.test.mjs` now stands as the permanent regression guard that fails loudly if any future subcommand introduces a HEAD-moving git op. OPERATIONS.md § Sub-agent dispatch absorbs the "never `git checkout <ref>` on the consumer working repo" doctrine for new subcommand authors. Status: remains `applied`.
