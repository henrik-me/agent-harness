...
**Finding:** CS85 replaced the naive stripper with a CommonMark-correct explicit forward scan (`stripInlineCode`, no regex backtracking) in `scripts/check-clickstop-link-durability.mjs`, but the fix landed ONLY in that guard. The sibling guard `scripts/check-doc-xref-resolvability.mjs` (CS81) still carries the identical naive `/`[^`]*`/g` at line ~251, used by its live checks (b) cross-file anchor resolvability and (c) relative-link deliverability. The two guards now diverge in correctness for the same sub-problem. Candidate follow-up: extract the hardened `stripInlineCode` into a small shared, unit-tested module (node-builtins-only, review-gate-clone-safe) and have both guards import it — retiring the naive copy. Low severity / no known live failure today (CS81 is green; the gap affects only backtick-wrapped example references), so this is consistency debt, not an active bug.

**Evidence:** `scripts/check-doc-xref-resolvability.mjs:251` (`lines[i].replace(/`[^`]*`/g, '')`; used at :315, :381); `scripts/check-clickstop-link-durability.mjs` `stripInlineCode` (the correct forward scan, CS85). Surfaced by CS85 review rounds R2/R3/R4 (each a Needs-Fix on exactly this inline-code class) — see `done_cs85_consumer-doc-clickstop-link-durability.md` and LRN-180 (point c).
