...
**Finding:** **Lock the bare-model-id rule into REVIEWS.md § 2.7 (Review log schema) and add a regression test alongside `tests/cs51-review-gates-logic.test.mjs` (which covers `scripts/checks/check-review-log-evidence.mjs`) that fails when `Model = 'gpt-5.5 (reviewer)'`.** The fix is purely documentation + test:
