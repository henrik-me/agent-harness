...
Run `node bin/harness.mjs sync --mode=check` (validates your config against the
schema) and `node bin/harness.mjs lint` to confirm conformance before reporting
back.
...
by `node bin/harness.mjs dispatch` from the harness-owned `DISPATCH-PREAMBLE.md`) verbatim into every sub-agent
