...
- `scripts/checks/check-copilot-review-attached.mjs` — own `loadReviewsConfig` (L50): fails closed (throws `UsageError`) only on **invalid JSON**, but on parseable input returns `cfg.reviews` whenever `typeof cfg.reviews === 'object'` (L55) — which **accepts arrays and other non-plain-object shapes**, falling back to `{}` only when `reviews` is absent/null/primitive (L52/L55). **Shape-lenient**: the LRN-145 rule-1 anti-pattern for malformed-shape inputs.
...
| R3 | gpt-5.5 | claude-opus-4.8 | rubber-duck (orchestrator: `yoga-ah`) | eae5c8400207 | 2026-06-05T16:14:24Z | Go | Narrow re-attest: tightened C61-3c fact-claim (loadReviewsConfig accepts any typeof==='object' incl. arrays) per Copilot; verified vs source, no contradictions. |
