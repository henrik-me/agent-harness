...
| T4 | implement `tests/check-review-output.test.mjs` (≥10 cases per Deliverable 2) | done | yoga-ah | 16 cases passing — original 12 + R1 amendment added independence-violation (fake-gh) + --update-pr idempotency (fake-gh round-trip) + Copilot-review amendment added Makefile/LICENSE/Dockerfile extensionless-file enumeration regression + $-pattern preservation regression in --update-pr |
