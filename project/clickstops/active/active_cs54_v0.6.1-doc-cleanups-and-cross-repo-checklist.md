...
- `tests/cs51-review-gates-logic.test.mjs` — new regression test for LRN-136 with **8 sub-cases** (R1 + R3 + R5 evolution): `gpt-5.5 (R2)` with populated fallback, `gpt-5.5 (PvI)`, bare-positive control, `gpt-5.5 (reviewer)`, Needs-Fix-row decoration (uniform enforcement), three non-parenthesized forms (`gpt-5.5 R2` / `gpt-5.5 - R2` / `gpt-5.5/R2`), and the display-form anti-suggestion sub-case (`Claude Opus 4.7` must reject AND must NOT emit `use bare "Claude"`).
...
| T5 Lock Review log Model bare-id rule (LRN-136) | done | omni-ah | REVIEWS.md § 2.8 Review log column rules paragraph + bare-id check inserted BEFORE reviewerModelApproved() in scripts/checks/check-review-log-evidence.mjs + regression test added to tests/cs51-review-gates-logic.test.mjs ("rejects decorated reviewer model identifiers (LRN-136)" suite covers 8 assertion sub-cases: bare-positive, parens decorations, whitespace decorations, slash decoration, Needs-Fix-row enforcement, display-form anti-suggestion). Targeted file `node --test` reports 8/8 it-blocks pass; full suite 1021/0/1. Did NOT touch check-independence-invariant.mjs. LRN-136 status applied. |
