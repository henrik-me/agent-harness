...
|---|---|---|---|
| C37-1 | Spike-first | Deliverable #1 is a verification spike: resolve `/users/copilot-pull-request-reviewer` actual node ID (handling User vs Organization vs Bot/App identity shapes); run `requestReviews` against a sandbox PR; capture every Copilot review-state event for at least one full cycle (request → submit → re-request); document in `docs/adr/ADR-NNN-copilot-graphql-spike.md`. Spike outcome is one of: **PASS** (identity resolves, mutation works, all observable states), **PARTIAL** (identity + mutation work but some review states unobservable / timing unreliable), **FAIL** (identity unresolvable OR mutation rejected OR fundamental shape mismatch). | Per rubber-duck finding 4: don't build a linter on assumed shapes. Three-outcome ladder lets downstream CSs make precise conditional choices. |
| C37-1b | **Degradation contract (PROPAGATES TO CS38a/CS38b/CS39/CS41).** | If spike = **PASS**: full A5 + A16 ship; downstream CSs treat both as enabled. If spike = **PARTIAL**: A16 ships (presence check), A5 ships only if review-state ordering is observable; otherwise A5 deferred to v0.5.0 follow-up CS. If spike = **FAIL**: BOTH A5 + A16 degrade to doctrine-only stubs (linters exist but always exit 0 with a "deferred to v0.5.0; see ADR-NNN" notice); follow-up CS filed for v0.5.0; CS38a's `review_gates.gate_set` MUST exclude the deferred gates from default; CS38b's retro acceptance threshold drops to ≥3 failures (B1+A3+A4); CS39's release notes MUST NOT advertise the deferred gates as enforced (only as doctrine); CS41's `depends_on` becomes `(CS37 PASS) OR (follow-up GraphQL CS landed)`. The orchestrator records the outcome in CS37's close-out notes and surfaces an ESCALATION line in the close-out report so downstream CSs are claimed with the correct conditional state. | Per GPT-5.5 review BLOCKING #1: degradation must propagate or arc falsely claims enforcement. Each downstream CS now has an explicit branch. |
...
|---|---|---|---|
| T1 | Pre-CS37 spike: resolve `copilot-pull-request-reviewer` actual node ID; capture identity shape (User / Bot / Mannequin); record GraphQL viewer token scopes | pending | orchestrator | Spike scratch: `gh api graphql -F query='query { user(login: "copilot-pull-request-reviewer") { __typename, id } }'` and try Bot/Mannequin variants if User fails. Captured to ADR-NNN. |
| T2 | Pre-CS37 spike: dispatch `requestReviews` mutation against a sandbox draft PR (one-line README change in this repo); capture mutation payload + response; close draft after | pending | orchestrator | If mutation rejected, capture the exact error code (e.g., `COULD_NOT_RESOLVE_TO_NODE`) for ADR. Sandbox PR is closed without merge. |
| T3 | Pre-CS37 spike: observe the Copilot review-state lifecycle on the sandbox PR (PENDING → COMMENTED/APPROVED/CHANGES_REQUESTED); capture all observable transitions + the time delay; record `commit.oid` shape | pending | orchestrator | Critical for A5 ordering enforcement. If review never submits or stays PENDING forever, that's PARTIAL/FAIL outcome. |
| T4 | Author `docs/adr/ADR-NNN-copilot-graphql-spike.md` with PASS/PARTIAL/FAIL verdict + recommendation for downstream CSs | pending | orchestrator | HARD GATE per Exit Criterion #1. Verdict drives all subsequent work. |
| T5 | If PASS or PARTIAL: dispatch SA-1 (`bot37-graphql-lib`) — owns `lib/github-graphql.mjs` + `tests/lib-github-graphql.test.mjs` | pending | orchestrator → SA-1 | Pattern mirrors `lib/detect-repo-tier.mjs` per C37-3. Sequential — must complete before SA-2 starts. |
| T6 | If PASS: dispatch SA-2 (`bot37-copilot-linter`) — owns `scripts/check-copilot-review.mjs` + `tests/check-copilot-review.test.mjs` + `bin/harness.mjs` aggregator wiring (cmdPrEvidence) | pending | orchestrator → SA-2 | A5 + A16 implementation per C37-4/5/6. |
| T7 | If PARTIAL: SA-2 ships A16 (presence) only; A5 (ordering) deferred to v0.5.0 follow-up CS in `project/clickstops/planned/` | pending | orchestrator | Per C37-1b degradation contract. Records the deferral in CHANGELOG. |
| T8 | If FAIL: SA-2 ships only the doctrine-only stub for `scripts/check-copilot-review.mjs` that always exits 0 with a "deferred to v0.5.0" notice; file follow-up CS | pending | orchestrator | Per C37-1b degradation contract. CS38a/CS38b/CS39/CS41 must adjust their conditional logic. |
| T9 | OPERATIONS.md § Copilot engagement procedure: capture the working invocation from spike (verbatim transcript); cross-link to new linter | pending | orchestrator | Composed-blocks parity: root + template/composed must be byte-equivalent. |
| T10 | CHANGELOG.md `[Unreleased] / Added` entry — for full PASS, lists all three (GraphQL primitive, A5 gate, A16 gate); for PARTIAL/FAIL, lists only what shipped + deferral note | pending | orchestrator | Per C37-1b. Pre-validate body with `node scripts/validate-schemas.mjs`. |
| T11 | Local validation: `harness lint --quiet` (27/0/3 baseline), `harness sync --mode=check` clean, `node --test tests/*.test.mjs` total ≥785 + new tests | pending | orchestrator | Per Exit Criteria #4 + #5. |
