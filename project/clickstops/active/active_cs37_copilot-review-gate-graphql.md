...
|---|---|---|---|
| T1 | Pre-CS37 spike: resolve `copilot-pull-request-reviewer` actual node ID; capture identity shape (User / Bot / Mannequin); record GraphQL viewer token scopes | done | orchestrator | Spike scratch: `gh api graphql -F query='query { user(login: "copilot-pull-request-reviewer") { __typename, id } }'` and try Bot/Mannequin variants if User fails. Captured to ADR-NNN. |
| T2 | Pre-CS37 spike: dispatch `requestReviews` mutation against a sandbox draft PR (one-line README change in this repo); capture mutation payload + response; close draft after | done | orchestrator | If mutation rejected, capture the exact error code (e.g., `COULD_NOT_RESOLVE_TO_NODE`) for ADR. Sandbox PR is closed without merge. |
| T3 | Pre-CS37 spike: observe the Copilot review-state lifecycle on the sandbox PR (PENDING → COMMENTED/APPROVED/CHANGES_REQUESTED); capture all observable transitions + the time delay; record `commit.oid` shape | done | orchestrator | Critical for A5 ordering enforcement. If review never submits or stays PENDING forever, that's PARTIAL/FAIL outcome. |
| T4 | Author `docs/adr/ADR-NNN-copilot-graphql-spike.md` with PASS/PARTIAL/FAIL verdict + recommendation for downstream CSs | done | orchestrator | HARD GATE per Exit Criterion #1. Verdict drives all subsequent work. |
| T5 | If PASS or PARTIAL: dispatch SA-1 (`bot37-graphql-lib`) — owns `lib/github-graphql.mjs` + `tests/lib-github-graphql.test.mjs` | done | orchestrator → SA-1 | Pattern mirrors `lib/detect-repo-tier.mjs` per C37-3. Sequential — must complete before SA-2 starts. |
| T6 | If PASS: dispatch SA-2 (`bot37-copilot-linter`) — owns `scripts/check-copilot-review.mjs` + `tests/check-copilot-review.test.mjs` + `bin/harness.mjs` aggregator wiring (cmdPrEvidence) | done | orchestrator → SA-2 | A5 + A16 implementation per C37-4/5/6. |
| T7 | If PARTIAL: SA-2 ships A16 (presence) only; A5 (ordering) deferred to v0.5.0 follow-up CS in `project/clickstops/planned/` | n/a | orchestrator | Per C37-1b degradation contract. Records the deferral in CHANGELOG. |
| T8 | If FAIL: SA-2 ships only the doctrine-only stub for `scripts/check-copilot-review.mjs` that always exits 0 with a "deferred to v0.5.0" notice; file follow-up CS | n/a | orchestrator | Per C37-1b degradation contract. CS38a/CS38b/CS39/CS41 must adjust their conditional logic. |
