...
| R2 | **CHANGELOG section not extractable** — `release.yml`'s awk extractor (`.github/workflows/release.yml:52-55`) matches `^## \[<version>\]` and stops at the next `^## \[`. If the new `## [0.6.0]` heading is mis-formatted (wrong brackets, typo, missing `## `), `release-notes.md` ends up empty and the workflow fails the `[ -s release-notes.md ]` check. The em-dash itself is NOT extractor-required; em-dash is a **project convention** per C53-2 (R1 corrected this). | Pre-tag dry-run: `awk '/^## \[0\.6\.0\]/ { found=1; print } END { exit found ? 0 : 1 }' CHANGELOG.md` must exit 0 and print the header line. Additional convention check: `Get-Content CHANGELOG.md \| Select-String -Pattern '## \[0\.6\.0\] — '` must return one match (em-dash present per project convention). |
