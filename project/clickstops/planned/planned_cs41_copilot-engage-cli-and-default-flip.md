...
**Depends on:** [CS40](planned_cs40_check-review-output-linter.md), [CS37](planned_cs37_copilot-review-gate-graphql.md) (GraphQL primitive — required), [CS35](planned_cs35_enforcement-doctrine-and-planning-locality.md).
...
The implementer-not-reviewer linter closes gap #5: PR #28's `## Model audit` had the same agent in both columns; A3 (CS36) catches model overlap but not agent-identity overlap. CS35 documents the doctrine; this CS mechanizes it.
...
| C41-6 | Where to source agent identity | Option A: parse from a sibling column in `## Model audit` (`Implementer agent`/`Reviewer agent` columns). Option B: derive from commit authors (B1 trailer + git log). **Choose A** — explicit columns, no inference. Update REVIEWS.md schema in this CS to add the columns. | Inference from git is fragile; explicit columns are auditable. Add columns now (CS41 deliverable) — justified because we're shipping a default-flip release where consumers are expected to refresh anyway. |
...
6. **REVIEWS.md** + **template/composed/REVIEWS.md** updates: add `Implementer agent` + `Reviewer agent` columns to `## Model audit` schema (per C41-6). Migration note: existing PRs without these columns produce a warning (not error) for one cycle; CS42 release-cut may upgrade to error.
7. **`harness.config.json`** schema + `bin/harness.mjs init` + `bin/harness.mjs sync` updates per C41-7/8.
8. **`tests/sync-review-gates-default-flip.test.mjs`** (new): verify default-flip behavior — fresh init writes enabled:true; sync errors on missing block; sync accepts explicit opt-out with reason.
9. **OPERATIONS.md**: § Copilot engagement procedure refresh — replace manual GraphQL recipe with `harness copilot-engage` invocation.
10. **CHANGELOG.md** `[Unreleased] / Added` (CLI + linter) + `[Unreleased] / Changed` (default flip) entries.
...
- **SA-2 (`bot41-impl-not-reviewer-and-flip`)** — owns the implementer-not-reviewer linter (`scripts/...` + `tests/...`) AND the default-flip work (schema, init, sync, sync test, REVIEWS.md schema columns).

Orchestrator owns OPERATIONS.md + CHANGELOG.md + REVIEWS.md prose (not column additions — those go with SA-2's schema work).
...
4. REVIEWS.md `## Model audit` schema includes `Implementer agent` + `Reviewer agent` columns; existing PRs continue to work (warn, not error).
5. `node --test tests/*.test.mjs` total = prior + ≥11.
6. `harness lint --quiet` + sync drift on the harness repo: passes (the harness's own `harness.config.json` already has `review_gates.enabled: true` from CS38b).
