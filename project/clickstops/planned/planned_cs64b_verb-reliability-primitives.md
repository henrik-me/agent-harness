...
**Depends on:** **CS64** (hard) — the doctor / disposer / `upgrade`-reconciliation strands harden or extend verbs that CS64 introduces (`startup`/`status`, the cross-clone temp-dir/clone patterns shared by the lifecycle verbs, and the `harness upgrade` flow), so CS64 must close first. The governance-doc strand is **CS64-independent**: C64b-7 (seed/init delivery) can split into an earlier wave alone, but C64b-8 couples to C64b-3 (it reuses `--apply-new` and edits the same `lib/sync.mjs` surface), so C64b-8 should carry `--apply-new` with it or land after C64b-3.
