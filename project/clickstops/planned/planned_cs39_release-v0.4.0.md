...
| C39-3 | Release notes scope | Highlight: planning-locality enforcement; `harness pr-evidence` entry point + B1/A3/A4/A5/A16 gates; reviewer model fallback ladder; PR template managed→composed migration; `--enable-review-gates` opt-in. Migration callout: existing consumers see a sync warning until they opt in. | Consumer-facing summary of the arc. |
