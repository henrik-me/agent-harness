...
- **D54-3** — **Lock the Review log Model column bare-id rule via a regression test** (LRN-136), not just docs. A row like `| Reviewer model | Claude Opus 4.7 |` (display name with spaces) silently passes today; only normalised bare ids should match the model audit independence check.
...
- `scripts/check-review-evidence.mjs` — tighten the `## Review log` `model` column parser (around L348-L419) to REJECT decorated values like `gpt-5.5 (reviewer)`, `gpt-5.5 (R2)`. Emit error: `## Review log row N: decorated model identifier "<value>"; use bare "<bare>" and put annotations in the actor column`. Independence check stays unchanged.
- `tests/check-review-evidence.test.mjs` — new regression test for LRN-136: a fixture PR body with `| ... | gpt-5.5 (R2) | ... |` in `## Review log` is REJECTED with the decorated-identifier error. Adjacent test confirms `gpt-5.5` (bare) still passes.
- `LEARNINGS.md` — LRN-134/135/136 transitioned `open` → `closed` with `applied_in_cs: CS54` and a 1-paragraph disposition each linking to the deliverable section above. LRN-136 disposition MUST correct the stale script/test path reference (LRN-136 currently cites `tests/check-review-log-evidence.test.mjs` — actual file is `tests/check-review-evidence.test.mjs`).
...
- Tighten `scripts/check-review-evidence.mjs` (around L348-L419 — `## Review log` row parser): for each data row, if the `model` cell matches `/\s*\(.*\)\s*$/`, emit error `## Review log row N: decorated model identifier "<value>"; use bare "<bare>" and put annotations in the actor column`.
- Add regression coverage in `tests/check-review-evidence.test.mjs` (the actual file — LRN-136's `tests/check-review-log-evidence.test.mjs` reference is stale, fix the LRN disposition simultaneously): two new test cases — (a) fixture with `| ... | gpt-5.5 (R2) | ... |` REJECTED with the decorated-identifier error; (b) fixture with bare `gpt-5.5` still passes.
