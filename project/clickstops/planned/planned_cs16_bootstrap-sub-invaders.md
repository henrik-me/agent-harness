...
**Depends on:** CS22 (`v0.2.0` published tag — the pin target; supersedes CS14 as the pin target now that v0.2.0 captures the full post-CS14 delta), CS15a (public flip + Ruleset shape proven), CS15e (constraint-detection flow), CS10 (scaffolds)
...
| C16-2 | harness pin in sub-invaders | `v0.2.0` (string) in `harness.config.json:version` | Latest published tag at CS16 claim time (cut by CS22). Captures the full post-CS14 delta — bootstrapping on v0.1.0 would commit sub-invaders to a stale shape and waste SI-CS03's pin-bump on a single-step jump. |
...
| **C16-14** | **Azure resource provisioning** | **Documented `az` CLI commands; user runs them at the explicit user-approval gate.** Resources: 1 resource group, 1 Storage Account (Tables for leaderboard + sessions), 1 SWA (free tier; managed-Functions enabled) — see § "User-approval gates" for the exact command set. | User directive: "deployment approvals etc. is needed" — Azure resource creation is the canonical "deployment approval" step. |
...
| **G4** | **Azure resources provisioning + spend guardrails** | After bootstrap PR merged, **before SI-CS01 claim** (SI-CS01 is the first SWA-deploying CS) | User runs `infra/provision.sh` locally with their Azure credentials; the script creates RG + Storage + SWA. **The script also creates an Azure Budget** (`az consumption budget create`) on the resource group with a **monthly cap of $5** and an **email alert at 50% / 80% / 100%**. The script also creates an **Action Group** for the alerts. SWA-create returns the deploy token; user pastes it into sub-invaders' GHA secrets as `AZURE_STATIC_WEB_APPS_API_TOKEN`. | Real Azure spend (free tier targeted; SWA Free + Storage LRS ≈ $0–$1/mo for v1 traffic). Budget alert is the safety net against runaway cost from a stuck Function or DDoS-style abuse. |
...
|---|-----------|-------------|
| 1 | `cs16-architecture-author` | `agent-harness:project/clickstops/active/active_cs16_*/architecture-source.md` (drafts the full Sub Invaders ARCHITECTURE.md content per Decisions #6–7 + C16-9..12) |
