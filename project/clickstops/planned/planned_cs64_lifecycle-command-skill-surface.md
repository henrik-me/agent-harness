...
- **G-skill** — post-spike decision: go/no-go on shipping runtime-skill wrappers, **informed by the C64-9 spike's silent-skip mitigation evidence**. The gate cannot return "go" unless the proposal documents a workable silent-skip mitigation; absent that, "no-go" is the only valid outcome. If "go", a follow-up CS files the wrapper shipment per C64-9.
