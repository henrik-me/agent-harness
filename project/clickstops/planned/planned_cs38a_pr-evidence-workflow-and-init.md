...
| C38a-4 | PR template migration | New file class transition: schema `composed.overrides[]` array gains an `_inherited_class` field allowing `managed → composed` with explicit migration notice in CHANGELOG. Migration sync test verifies existing consumer config + new content roundtrip cleanly. | Per rubber-duck finding 2: don't silently change file class. Explicit migration with sync test. |
...
| C38a-6 | `review_gates` config schema | `harness.config.json` gains `review_gates: { enabled: bool, copilot_required: bool, gate_set: ["B1","A3","A4","A5","A16"] }`. Default if absent: `{ enabled: false, copilot_required: false, gate_set: [] }` (opt-in for v0.4.0 per C35-15). `harness init --enable-review-gates` writes `{ enabled: true, copilot_required: true, gate_set: all }`. | Explicit field; sync warns on missing in v0.5.0; CS41 flips the default. |
...
| C38a-10 | Bot / fork / workboard label predicates | Workflow `if:` clauses skip `read-only-gates` job entirely on (a) `workboard-only` label, (b) author in `dependabot[bot]`/`github-actions[bot]`. Fork-source PRs run only B1+A3+A4 (A16 fails with exit 2 → workflow records as "skipped (fork)"). | Per C35-7/8/9. |

## Deliverables

1. **`template/managed/.github/workflows/pr-evidence-lint.yml`** (new): two jobs per C38a-2/3/10; uses `actions/checkout@v4` with `fetch-depth: 0` (B1 needs full history); runs `npx harness pr-evidence` against PR base/head/body.
