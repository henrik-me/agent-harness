...
| C57-3 | High-risk source of truth | Read the high-risk set from `harness.config.json` → `reviews.high_risk_clickstops` (already schema-defined and populated). Compare CS IDs case-insensitively (upper-cased). Use the documented default `["CS03","CS11","CS15a","CS18b","CS19"]` ONLY when the value is **not an array** — i.e. the config file is absent, or the file is valid JSON but the `reviews.high_risk_clickstops` key is **absent (missing)**. An explicitly empty array (`[]`) is **honored as an empty high-risk set** (no CS treated as high-risk) — it is NOT replaced by the default. (Present-but-non-array / malformed JSON is handled by C57-6.) | Closes the LRN-039 config-vs-code drift, and matches the empty-array/absent-key semantics of the existing config consumers EXACTLY: `lib/review.mjs:291-293` and `bin/harness.mjs:721-723` use `Array.isArray(...) ? value : default`, and `scripts/checks/check-independence-invariant.mjs:141` uses `value ?? default` — all three honor any array value (including `[]`) and fall back to their default only when the value is absent (the per-consumer default content differs: `[]` in `lib/review.mjs`, the CS-list in the other two — C57 uses the documented CS-list default). A missing key is the normal "consumer hasn't customized it" state → default; an intentional `[]` is a deliberate "no high-risk CSs" choice and must be respected, not silently overridden. The DEFAULT constant keeps the linter sane out of the box. |
...
1. **`scripts/check-clickstop-implementer-not-reviewer.mjs`** — extend per C57-1..C57-6:
   ...
   - `loadHighRiskClickstops()` reading `reviews.high_risk_clickstops` from `harness.config.json`: documented-default fallback ONLY when the value is not an array (absent file or absent key); an explicit empty array (`[]`) honored as an empty high-risk set; **fail-closed** (error + non-zero exit) on JSON parse error or present-but-non-array value (C57-3, C57-6).
...
| R-config | C57-6 fail-closed on malformed config is a behaviour change vs PR #201 (which defaulted). A consumer with a malformed `harness.config.json` would newly fail this linter. | Scoped per the GPT-5.5 plan review: fail-closed applies ONLY to a JSON parse error or a present-but-non-array value; a missing (absent) key uses the schema default, and an explicit empty array (`[]`) is honored as empty — both never error (C57-3, matching the existing consumers `lib/review.mjs:291-293` / `bin/harness.mjs:721-723` / `check-independence-invariant.mjs:141`). Error message names the file + key. Matches LRN-033. |
