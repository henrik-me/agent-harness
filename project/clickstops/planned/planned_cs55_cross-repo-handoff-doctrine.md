...
- **D55-3 — Uniform default label:** Every handoff issue MUST always carry the `harness-orchestrator` label as the routing default. Supplemental labels (e.g. `harness-sync`, `release-blocker`) are permitted as additions and never replace or remove the default. **Label preflight prerequisite (Copilot R5):** if the target repo does not yet have the `harness-orchestrator` label, the orchestrator MUST create it idempotently (e.g. `gh label create harness-orchestrator --repo OWNER/NAME --color 0E8A16 --description "Filed by harness orchestrator" --force`) BEFORE running `gh issue create --label harness-orchestrator`; otherwise `gh issue create` fails with "label not found" and the doctrine breaks on first-time consumer repos. CS56's CLI automates this preflight; manual `gh issue create` invocations (e.g. CS55's T6) must do the same. Rationale: one predictable v1 routing label is enough for triage, while CS56's `--label` repeated flag can append context labels without weakening discoverability; idempotent label provisioning keeps the issue-only happy path working for newly-onboarded consumer repos.
...
- `OPERATIONS.md` (root mirror, **composed**, classified at `harness.config.json:31`) — regenerated from the updated `template/composed/...` source via `harness sync --mode=apply` so the live `## Cross-repo procedures` section is present at the root for both human and tooling consumers.
...
- After T2 (template/managed/...) and T3 (template/composed/...) edits are committed in the content branch, run `harness sync --mode=apply` from the repo root to re-render the root mirrors. `OPERATIONS.md` is composed (`harness.config.json:31`) and `.github/copilot-instructions.md` is managed (`harness.config.json:22`).
- Verify the rendered roots actually contain the new Hard Rule § 6 and the new `## Cross-repo procedures` section before committing.
...
- **Exit criteria:** `git diff` after `harness sync --mode=apply` is empty (mirrors in sync); `OPERATIONS.md` and `.github/copilot-instructions.md` at repo root carry the new content.
...
- **Label preflight (D55-3, Copilot R5):** before `gh issue create --label harness-orchestrator`, ensure the label exists in `henrik-me/sub-invaders`: `gh label create harness-orchestrator --repo henrik-me/sub-invaders --color 0E8A16 --description "Filed by harness orchestrator" --force`. The `--force` flag makes `gh label create` idempotent (it updates an existing label rather than erroring). Without this preflight, `gh issue create --label harness-orchestrator` exits non-zero with "label not found" if SI has not yet adopted the label.
