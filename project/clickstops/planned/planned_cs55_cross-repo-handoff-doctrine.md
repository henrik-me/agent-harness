...
- Include subsection (f) **Reconcile with C35-13 (no-internal-issues rule):** the existing `OPERATIONS.md` text "Issues filed by the agent are forbidden (Decision C35-13). GitHub issues are an INBOUND channel ... but never opens them." applies ONLY to the harness repo itself (planning is via CSes, not issues). Cross-repo handoff issues to OTHER repos (e.g. `henrik-me/sub-invaders`) are the ONLY allowed write op in those repos under Hard Rule § 6. The content PR MUST amend that paragraph (currently at `template/composed/OPERATIONS.md:91-93` and root mirror `OPERATIONS.md:91-93`) to add a scoping parenthetical such as "(applies to the harness repo only — see `## Cross-repo procedures` for issue-only handoffs to other repos)" so the two rules cannot be read as contradictory.
