...
- **Also resolves inbound issue [#497](https://github.com/henrik-me/agent-harness/issues/497)** (triaged 2026-07-05 by `yoga-ah`): #497 **Problem 1** (add a `pull_request_review` trigger to `review-gates.yml` so `copilot-review-attached` / `independence-invariant` re-evaluate on review submission, matching `pr-evidence-lint.yml`) is delivered by **C90c-7**'s trigger port. #497 **Problem 2** (honor the `workboard-only` skip robustly regardless of label timing) is already satisfied at HEAD — `review-gates.yml` triggers on `labeled`/`unlabeled` and its evidence jobs run on every PR and skip at the *step* level (post-CS71, LRN-218) — and is reinforced by **C90c-2**'s full skip-reason set. Caveat: LRN-194 notes a `pull_request_review` trigger does NOT fire for the Copilot **bot**'s own review (only human reviews), so the async Copilot-review rerun remains a separate gap tracked by LRN-194 / LRN-217; #497 Problem 1's explicit ask (the trigger itself) is nonetheless met.
