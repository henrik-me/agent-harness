...
| C64-4 | `harness claim <NN>` = workboard-claim step ONLY | Backed by `lib/claim.mjs`: preflight (clean worktree; exactly one `planned_cs<NN>_*` match; `main` up to date; deterministic slug from the planned filename; target `cs<NN>/claim` branch absent), run the `lib/harvest.mjs` pre-claim gate (blocking — non-zero exit refuses claim; bypass with `--skip-harvest`), create branch `cs<NN>/claim`, add the `WORKBOARD.md` Active-Work row, `git mv planned→active`, and **print the commit + `workboard-only` PR instructions**. It does **not** create the `cs<NN>/content` branch (that is a separate, post-claim-merge step) and does **not** commit/push automatically. `--dry-run` prints the mutation plan; apply is explicit; idempotent (already-active CS ⇒ no-op with a clear message). | Conforms to the three-PR shape (`OPERATIONS.md § Claim — Three-PR shape`): claim mutations belong on `cs<NN>/claim` in a `workboard-only` PR, separate from content work. Preflights + dry-run + idempotency prevent half-applied WORKBOARD/branch state. |
...
| R5 | gpt-5.5 | claude-opus-4.7-1m-internal | rubber-duck (orchestrator: omni-ah) | a49a4a405338 | 2026-06-10T16:10:00Z | Go | Re-attestation: harvest gate cells C64-4 + T2 corrected from "advisory" to "blocking" to match implementation (lib/claim.mjs runClaimFromDisk fails closed on harvest non-zero). No behavioral drift. |
...
| T11: Resolve G-skill gate per T8 outcome — record go/no-go in active CS file; if go, file follow-up CS for wrapper shipment; if no-go, record silent-skip mitigation gap as rationale. Depends on T8. | done | omni-ah | agent-id=cs64-g-skill \| role=implementer \| report-status=delivered \| learnings=0 \| NO-GO recorded in `runtime-skill-spike.md` with silent-skip mitigation rationale (PR #289 `51953da`) |
