...
|---|---|---|---|---|
| 1 | `ajv`/`ajv-formats`/`js-yaml` declared as devDeps only → `ERR_MODULE_NOT_FOUND` on every consumer install | HIGH | **FIXED** in v0.3.0 (CS25, `b37a22f`) | **No code work** — add a CHANGELOG cross-reference + a sub-invaders-bootstrap-summary update note (now archived at `project/clickstops/done/done_cs16_bootstrap-sub-invaders/sub-invaders-bootstrap-summary.md`) |
