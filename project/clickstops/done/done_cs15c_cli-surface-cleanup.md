...
2. **CS04d:** Reject `harness sync --ref <X>` and `harness check --ref <X>` with exit 2 + a documented "not yet implemented" message ([Option B chosen](./done_cs04d_global-ref-flag-wiring.md) per the planned file's recommendation; Option A — full ref-pinning impl — deferred to a future CS that designs git-ref-based template fetching properly).
...
|---|---|---|
| [CS04b](./done_cs04b_thread-config-flag-through-sync.md) | LRN-027 (CS04 close) | `--config` threading + 3 error paths |
| [CS04d](./done_cs04d_global-ref-flag-wiring.md) | CS04 R1 NB-2 | `--ref` Option B reject + help-text update |
| [CS09b](./done_cs09b_sync-fixture-extension.md) | LRN-057 (CS09 close) | `harness sync --mode=check` step in init fixture test |
...
| Close-out: docs + restart state (CONTEXT/WORKBOARD/HANDOFF + RESUME POINT in this file; rename active → done; `git mv` 3 absorbed planned files to `done/` with "absorbed by CS15c" pointer) | done | yoga-ah | renames at this PR; CONTEXT/WORKBOARD/HANDOFF updated in same commit |
| Close-out: learnings + follow-ups (LEARNINGS.md LRN-084..086; LRN-082/083 already taken by CS15a; document any deferred follow-ups as new planned CSs) | done | yoga-ah | LRN-084 (init→sync-apply finalize), LRN-085 (override-config error contract), LRN-086 (LRN-range reservations are advisory) filed |
...
**Reviewer:** GPT-5.5
**Date:** 2026-05-09
**Outcome:** GO (R2 final; R1 was NEEDS-FIX, addressed in commit `fa78147`)

Two-round GPT-5.5 plan-vs-implementation review per LRN-064. R1 returned NEEDS-FIX with 2 blockers (config-error message contract not fully satisfied; missing `check --config` regression test). Both addressed in commit `fa78147`. R2 returned GO. Full review records reproduced verbatim below.
