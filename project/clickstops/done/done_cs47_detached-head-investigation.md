...
**Dynamic evidence (`tests/cs47-detached-head-bisect.test.mjs`, 24/24 green):**
...
- The two LRN-124 live-repro subcommands — `lint` and `plan-review-hash` — plus `sync --mode=check`, `check`, `version`, `whoami`, and the three stubs (`harvest`, `check-migration`, `composed-audit`, which `die()` before any git) run in BOTH self-host and consumer modes (C47-4(c)). The **`sync --mode=apply`** path (the LRN-124 incident-#1 trigger) is exercised in self-host mode on a dedicated fresh clone, and **`init`** (which finalizes via `sync --mode=apply`) runs in BOTH consumer and self-host modes (the self-host run on its own dedicated clone, since `init` writes). A dedicated standalone test additionally scaffolds a real consumer with `init` (asserting `harness.config.json` materialized) and then runs explicit **`sync --mode=apply`** and **`sync --mode=check`** over that now-initialized consumer — closing the deep apply-path coverage gap on an initialized repo rather than the config-missing early-error path.
