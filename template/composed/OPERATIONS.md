...
`{{harness_invoke}} sync` (report-only) and adopted with `{{harness_invoke}} sync
...
`main`, before any implementation work. **How:** run `{{harness_invoke}} claim CS<NN>`
...
live in `{{harness_invoke}} claim --help`; use the directory form for
...
all live in `{{harness_invoke}} review-output --help`. The aggregator
...
`## Model audit`. **How:** run `{{harness_invoke}} review --help` for the full
...
Run `{{harness_invoke}} copilot-engage <pr-number>` to request the Copilot
...
codes live in `{{harness_invoke}} copilot-engage --help`. Key doctrine the
