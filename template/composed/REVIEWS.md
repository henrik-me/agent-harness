...
| F6 | Every **state-of-the-world claim** the plan relies on (e.g. "tag `v0.7.0` does not exist", "release X is in Draft", "PR Y is open", "label Z is present in the consumer repo", "the branch protection ruleset requires N approvals") is verified at plan-review time via a non-mutating CLI probe and the probe is recorded in the plan's Background or Constraints so subsequent reviewers can audit the same premise. For release/tag state: `gh release list --limit N`, `gh api repos/<owner>/<repo>/releases --jq 'map(select(.tag_name=="<tag>"))'` (both published AND draft), `git ls-remote origin refs/tags/<tag>`. For PR/issue/label state: `gh pr view <num> --repo <owner>/<repo>`, `gh issue view <num> --repo <owner>/<repo>`, `gh label list --repo <owner>/<repo>`. | GitHub API / git remote / live shell at plan-review time. |
