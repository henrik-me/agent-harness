...
- **Bootstrap sanity check:** before claiming any CS, run `harness startup`
  ...
  ```bash
  {{harness_invoke}} startup --pull-ff-only
  ```
  ...
  {{harness_invoke}} sync --mode=check --cwd .        # expect: "No drift detected"
...
- **Sub-agent briefing preamble**: every sub-agent dispatch MUST paste the canonical preamble —
  emitted verbatim by `{{harness_invoke}} dispatch` from the harness-owned `DISPATCH-PREAMBLE.md`
