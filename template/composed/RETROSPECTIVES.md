...
2. Run `{{harness_invoke}} lint` to confirm no errors remain.
...
5. After disposition, run `{{harness_invoke}} lint` before opening
   the claim PR.
...
```sh
{{harness_invoke}} lint           # runs the LEARNINGS.md check over the repo
{{harness_invoke}} lint --quiet   # summary only
```
