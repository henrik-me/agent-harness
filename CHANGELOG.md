...
- **CS70 filed (bootstrap-summary doctrine + stale-link cleanup):** New `project/clickstops/planned/planned_cs70_bootstrap-summary-doctrine-and-stale-links.md` files the planned fix for [`agent-harness#290`](https://github.com/henrik-me/agent-harness/issues/290) (Option 1 — declare the bootstrap summary harness-internal-only). Investigation traced the bug to CS16 close-out commit `40e464c` performing only a plan-file rename inside the directory-form CS, dropping the sibling `sub-invaders-bootstrap-summary.md` (created in `e2b233a`). At claim time — none of the following ships in this filing PR — CS70 will (a) restore the file at `project/clickstops/done/done_cs16_bootstrap-sub-invaders/sub-invaders-bootstrap-summary.md` with an Archive-note banner, (b) repoint the 4 stale-link CSes (CS25, CS27, CS30, CS26 — with the CS26 Deliverables L103 reference carved out for re-attest at CS26 claim time), (c) fix two CS16 self-references (L542 Type-A path repoint + L623 obsolete-doctrine appended-only supersession), (d) add `OPERATIONS.md` doctrine for directory-form CS close-out (`git mv` the whole dir, not per-file) and cross-repo target-verification (pre-flight check that the consumer file exists or is a contracted artifact), (e) extend `scripts/check-clickstop.mjs` with an orphan-detection check + per-CS `.harness-closeout-allow-drop` allow-list, and (f) file two LRNs (process: directory-form close-out file-loss mechanism; architectural: cross-repo phantom-artifact failure mode). 5-round GPT-5.5 plan review (R1 Needs-Fix → R2 Needs-Fix → R3 Go-with-amendments → R4 Go-with-amendments → R5 Go @ hash `d1c1de522470`). Filing only — implementation deferred to CS70 claim.
