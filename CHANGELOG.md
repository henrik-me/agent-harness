...
- **CS54 / T6:** New top-level section **`Config schema: reviews vs review_gates`** in `REVIEWS.md` disambiguating the two top-level `harness.config.json` blocks. `review_gates.*` is install-time + CI workflow (B1/A2..A6/A16 gate set); `reviews.*` is orchestrator-side `harness review` CLI + PR-side status checks. Field descriptions adapted from `schemas/harness.config.schema.json` (L107-200); the schema remains source-of-truth.
