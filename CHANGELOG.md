...
- **CS64b (verb reliability + consumer-delivery primitives — post-CS64 hardening):** A focused hardening pass applying LRN-151 / LRN-155 / LRN-157 and closing a consumer-delivery miss surfaced 2026-06-15 (the harness's core governance/onboarding docs reached **no** consumer — e.g. `sub-invaders` had none of the hard rules). Triggers a **minor** bump (new CLI surface + additive sync/init behaviour). Strands:
  ...
  - **`harness sync` new-managed-file reconciliation** (`lib/sync.mjs`, C64b-3): surfaces every `template/managed/` file absent from the consumer's `managed.files` (membership, not disk presence; sentinels such as `.gitkeep` excluded) as a report-only "new managed files" advisory alongside the existing drift detection; `--apply-new` adopts them (adds the `managed.files` entry + materializes the file).
  - **Required core managed-doc sync gate** (C64b-8): `harness sync --mode=check` (and the default path) emits a **WARN** for every core governance doc missing from `managed.files`, distinct from the C64b-3 report-only advisory. Warn-only this minor (escalation to a hard error is deferred to a future bump, mirroring the `review_gates` v0.4→v0.5 warn→error migration); the net-new **`sync --quiet`** flag suppresses it (and makes the pre-existing OPERATIONS.md § Sync `--quiet` claim accurate).
  - **Fresh-init core managed-doc delivery** (`bin/harness.mjs` init + `template/seeded/harness.config.json`, C64b-7): `harness init` now registers the core governance/onboarding docs — `INSTRUCTIONS.md`, `.github/copilot-instructions.md`, `TRACKING.md`, `RETROSPECTIVES.md`, `READMEGUIDE.md` — in `managed.files` (via the seeded config's new `managed` block and an init `addUnique` that also covers the scaffold-fallback path), so the unconditional init→`sync --apply` finalize renders + materializes them into every consumer. Adds the missing `templating.repo_slug` to the seeded config so the core docs render with no unresolved `{{…}}`. Recorded once in a new single-source-of-truth `lib/core-managed-files.mjs` constant consumed by init, the sync gate, and tests.
