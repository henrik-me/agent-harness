...
- **CS54 / T5 (LRN-136):** Harden `scripts/checks/check-review-log-evidence.mjs` against decorated reviewer-model identifiers. The gate now detects strings like `gpt-5.5 (R2)`, `gpt-5.5 (PvI)`, `gpt-5.5 (reviewer)` in the `## Review log` `model` column and rejects them with an explicit error pointing to the bare identifier and to REVIEWS.md § 2.8. Closes the silent-pass path where a decorated cell was normalised to `gpt-5.5-r2`, failed the primary-reviewer check, and the fallback-rationale path approved it. Regression test in `tests/cs51-review-gates-logic.test.mjs` (8/8 tests pass). Does NOT touch `check-independence-invariant.mjs`.
...
- **CS54 / T6:** New top-level section **`Config schema: reviews vs review_gates`** in `REVIEWS.md` disambiguating the two top-level `harness.config.json` blocks. `review_gates.*` is install-time + CI workflow (B1/A2..A6/A16 gate set); `reviews.*` is orchestrator-side `harness review` CLI + PR-side status checks. Field descriptions copied verbatim from `schemas/harness.config.schema.json` (L107-200) so the schema remains source-of-truth.
