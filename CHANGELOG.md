...
- **CS54 / T5 (LRN-136):** Harden `scripts/checks/check-review-log-evidence.mjs` against decorated reviewer-model identifiers. The gate now enforces the bare-id rule `/^[A-Za-z0-9._-]+$/` on the `## Review log` `model` column, rejecting any non-bare form — parenthesized (`gpt-5.5 (R2)`, `gpt-5.5 (PvI)`, `gpt-5.5 (reviewer)`) AND non-parenthesized (`gpt-5.5 R2`, `gpt-5.5 - R2`, `gpt-5.5/R2`) — with an explicit error pointing to the bare identifier and to REVIEWS.md § 2.8. Closes the silent-pass path where a decorated cell was normalised away from the primary reviewer and the fallback-rationale path approved it. Regression sub-cases in `tests/cs51-review-gates-logic.test.mjs` cover decorated + fallback (the historically-silent path), decorated without fallback, bare positive control, `(reviewer)` annotation, Needs-Fix row enforcement, and the three non-parenthesized decoration forms. Does NOT touch `check-independence-invariant.mjs`.
