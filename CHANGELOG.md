...
- **`check-config-placeholders` linter + seeded `.gitattributes` (CS26, Findings #3/#9 from the CS16 sub-invaders bootstrap):** `harness lint` gains a `config-placeholders` check that flags un-replaced `REPLACE_ME` identity placeholders left in a consumer's `harness.config.json`. It is JSON-aware — it scans string values under non-`_`-prefixed keys, so the instructional `_comment` is ignored and a consumer who filled every real field passes even with the comment retained. `harness init` also now seeds a `.gitattributes` (`* text=auto eol=lf` + binary overrides), so fresh Windows consumers stop hitting CRLF round-trip warnings on first commit. Because a fresh init intentionally ships identity placeholders, `config-placeholders` flags them by design until the consumer fills them in: a fresh scaffold is *structurally* lint-clean (`harness lint --skip config-placeholders`) plus this directive "fill in your identity" reminder (Minor).

- **`harness review --implementer-models` for non-CS branches (CS68, C68-3):** `harness review <pr>` now runs against a non-clickstop maintenance branch (`deps/<pkg>-<ver>` or `dependabot/…`), which carries no `cs<NN>/` id and no clickstop ledger to parse. On such a branch the implementer-model set is sourced from a new `--implementer-models <csv>` flag and/or the PR body's existing `## Model audit`, **unioned** with a hard superset invariant so the flag can never shrink or overwrite the audited implementer set; the independence guard (reviewer model ∉ implementer set) is preserved. `cs<NN>/` branches are unchanged — they still read implementer models from the matching clickstop file. This lets dependency/maintenance PRs use the canonical review-evidence tooling instead of hand-authored `## Model audit` / `## Review log` blocks (Minor).
...
- **`harness init` seeds a real `version` pin (CS26, Finding #2 from the CS16 sub-invaders bootstrap):** a fresh init now derives the running harness install's provenance (`resolveHarnessProvenance` — npx-cache → git → package version) and writes a normalized `version` into the consumer `harness.config.json` (a SemVer tag → else the full 40-hex `resolved_sha` → else `v${package.json version}`), replacing the misleading hardcoded `v0.1.0`. Fresh-init-only, so re-running `init` never mutates an existing config (LRN-057 / C41-8). Scope note: the original 6-finding CS16 bundle was narrowed to the four still real against `main` (#2/#3/#6/#9); #4/#5 were dispositioned obsolete (Minor).
...
- **Dependency-bump adoption procedure (CS68, C68-1/C68-2):** new `OPERATIONS.md § Dependency-bump adoption` documents the repeatable steps to adopt a Dependabot/dependency PR through the review-evidence gates on a `deps/<pkg>-<ver>` branch — re-create off `origin/main`, pin the patched version + regenerate the lockfile, run tests + `harness lint`, generate the review evidence via `harness review --implementer-models` (not by hand), obtain a GPT-5.5 rubber-duck `Go` + Copilot review, then owner-override squash-merge and supersede the bot PR — cross-linking the CS59 solo-orchestrator admin-merge doctrine (C59-3) and [LRN-081](LEARNINGS.md#lrn-081) rather than restating them. `deps/<pkg>-<ver>` is registered in the `INSTRUCTIONS.md` branch-naming convention (Documentation).
...
- **REVIEWS.md §2.4.3 "Leaner review sequencing" (#424):** codifies bounding the content-PR review loop — rubber-duck to Go before the first Copilot engage; batch fixes into minimal commits; after the final Copilot re-engage, resolve all threads and merge without further commits. A genuinely new *blocking* finding still takes the fix→re-attest→re-engage path, but Copilot re-raises of already-fixed items and non-blocking style nits are "resolve, don't re-fix".
...
- **CS65 (process-doc right-sizing):** Right-size the two always-loaded process docs without losing any procedure or breaking any `LRN-###`/heading anchor. `LEARNINGS.md` shrinks ~54% (aged entries archived via stub-redirect; every `LEARNINGS.md#lrn-…` anchor and bare `LRN-###` token still resolves). `OPERATIONS.md` (and its composed base) collapse command-backed executable step-lists (`claim`, `review-output`, `review`, `copilot-engage` recommended invocation) into "what / when / `{{harness_invoke}} <cmd> --help`" pointer stubs, backfilling the executable detail into the command `--help` text — so consumers get the pointer form on the next `harness sync`. All 113 `OPERATIONS.md` headings are preserved; the sub-agent briefing preamble (extracted live from `OPERATIONS.md` by `harness dispatch` — `lib/dispatch.mjs`) is left intact, as is the reviewer-preamble doctrine (`harness review` composes its prompt in `lib/review.mjs`, not from the doc).

### Documentation

### Fixed

- **CS93 (harness review clickstop lookup — #407):** Fix `harness review <pr>` aborting its non-dry-run path with "Could not find clickstop file for CS0N …" when the clickstop file exists (`--dry-run` appeared to work only because it short-circuits before the lookup). The resolver `findClickstopFile` (`lib/review.mjs`) zero-stripped the branch-derived id (`CS02` → `CS2` via `normalizeCsId`) and then substring-matched the raw zero-padded filename `…_cs02_…`, so nothing matched. It now normalizes **both** sides (padding-insensitive: `CS02`↔`cs02`, `CS2`↔`cs2`), additionally resolves **directory-form** clickstops (`…_cs<NN>_<slug>/<same>.md`, first-class since CS70), and reads stage directories **fail-closed** (discriminates `ENOENT` instead of an `existsSync` gate that also swallows `EACCES`), and **fails closed on an ambiguous >1 match** (listing the paths) instead of a `readdir`-order-dependent first hit — aligning it with the robust `lib/review-cs.mjs` `locateClickstop` reference. Patch (bug fix to a distributed CLI internal; no new flag/subcommand/schema).
- **CS92 (copilot-engage reliability):** Harden `harness copilot-engage` against two transient `gh` failure modes and make its success signal honest. (1) A single transient GraphQL/`gh` blip — a random HTTP 401 (not `auth-missing`), an HTTP ≥ 500, or a spawn/transport failure — no longer aborts the whole engage: the seam network calls (PR resolve, identity, reviewer add, requested-reviewers read, and the in-place poll read) now retry with bounded linear backoff (default 5 attempts), while `auth-missing`, `bad-input`, and permission/scope denials still fail fast (`lib/github-graphql.mjs` gains an enriched `GraphQLError` surface — `httpStatus`/`exitCode`/`stderr`/`transport` — and a single exported `isTransientGhError` predicate; LRN-161). (2) After the `gh pr edit --add-reviewer` add, the requested reviewers are read back and, on a silent no-op (exit 0 but `requested_reviewers` stays empty), the add is retried exactly once; a still-absent reviewer now fails fast as a typed `reviewer-not-requested` error (exit 6) instead of a slow, misleadingly-worded poll timeout, and an unreadable list surfaces as `reviewer-verify-unavailable` rather than a silent pass (LRN-160). (3) The result object/JSON gains an additive `verified` flag: the `--no-poll` early return is explicitly `verified: false` (reviewer requested, review not confirmed at HEAD) while a poll success stays `verified: true` (a Copilot review whose commit is the PR HEAD, submitted after the floor — the existing poll-path guarantee, unchanged); exit codes for all success paths are unchanged (LRN-173). Patch (reliability fix; no new CLI flag, no schema change, existing exit codes preserved).
...
- **CS82 (sync apply lock provenance under npx — #352-F2):** Fix `harness sync --mode=apply` silently recording placeholder lock provenance (`harness_ref: "unknown"`, an all-zero `resolved_sha`, and `version: "unknown"` scaffolds) when the harness runs from an `npx`/`npm` install, whose installed package tree has no `.git`. Provenance now derives via an ordered chain against the running install — **(1)** the install project's `node_modules/.package-lock.json` (the harness package's `resolved` `git+https://…#<sha>` URL + the requested ref — the source of truth under npx/npm), then **(2)** the install's own git checkout (self-host), then **(3)** fail-closed. Apply mode now throws `ESYNC_UNRESOLVED_PROVENANCE` and writes **no** lock rather than persist a placeholder; `--resolved-sha` still overrides only `resolved_sha` and cannot rescue an underivable `harness_ref`. `--mode=check`/`--mode=dry-run` are unchanged (they validate file drift, not provenance, so a pre-existing corrupt lock is not newly red-flagged), and `schemas/harness-lock.schema.json` is intentionally not tightened (that would fail-closed on reading an existing placeholder lock). Patch-level bug fix (no new CLI surface); rolls into the pending release.
