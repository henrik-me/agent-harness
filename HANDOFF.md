...
- `node --test tests/*.test.mjs` — all green (669+ post-CS06c/CS08c close)
...
- **CS01 → CS15e complete.** CS15a absorbed the public visibility flip and Ruleset application. Two CS15d follow-ups (CS06c, CS08c) closed 2026-05-10 — see bullets below.
- **Pre-CS16 backlog cleanup COMPLETE.** All three umbrella CSs (CS15c/d/e) filed 2026-05-10 to clear the 7 deferred planned CSs accumulated through CS01–CS15a, per user authorization 2026-05-09 ("umbrella seems good"), are now closed:
  ...
  - **CS06c** — Centralize remaining doc-schema primitives in `lib/doc-schema.mjs` (CS15d follow-up). Single-CS orchestrator-owned scope. **DONE 2026-05-10 @ `2d87579` (PR #101).** R1 GPT-5.5 GO; plan-vs-impl GO. Filed LRN-096 (test-file naming convention drift: `tests/doc-schema.test.mjs` predates the `lib-` prefix added in CS15d). Adds 4 exports: `headingAnchor`, `collectHeadings`, `collectH2Headings`, `extractSectionBody`. Refactors `check-{instructions,readme,clickstop}.mjs` to delegate. Suite 643→663 (+20).
  - **CS08c** — Extend `check-templates` markdown-context awareness for tilde fences + indented code blocks + N-backtick spans (CS15d follow-up). Single-CS orchestrator-owned scope. **DONE 2026-05-10 @ `b90d08b` (PR #103).** R1 NEEDS-FIX (indented-code stripping was global, masking real `{{ obj.x }}` violations in YAML at ≥4 indent) → fixed in `36d6dc5` (added `isMarkdown` flag derived from file extension, gated indented-code stripping behind it, added YAML regression fixture) → R2 GO. Plan-vs-impl GO. Filed LRN-097 (markdown-context stripping in multi-format linters must gate markdown-only constructs by file type to avoid false-negatives in non-markdown templates). Suite 663→669 (+6).
...
- **Next work:** **CS16 (Bootstrap Sub Invaders)** is the next mainline CS — the first downstream consumer of CS15e's constraint-detection flow. Claim once a planned file is filed (no planned file currently exists; it must be authored before claim). The two small CS15d follow-up CSs (CS06c, CS08c) are now both closed 2026-05-10, so the pre-CS16 backlog is fully empty.
- 17 linters in `harness lint` non-self-host (15 base + `templates` + `scaffold-readme` skipped row); **24 rows in self-host** (15 base + templates + 8 per-scaffold rows); **669 tests passing** (post-CS06c+CS08c, +26 net over CS15e); public/private-smoke workflow verified end-to-end against `v0.1.0` via `npx -y "github:henrik-me/agent-harness#v0.1.0"`.
