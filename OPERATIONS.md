...
live in `node bin/harness.mjs claim --help`; use the directory form for
...
1. Run `node bin/harness.mjs lint` — must exit 0 (it includes the
...
which `node bin/harness.mjs dispatch` machine-extracts and emits it verbatim (CS86
C86-2). Run `node bin/harness.mjs dispatch` and paste the emitted block — do not
hand-copy or reconstruct it. Per-language variants are emitted by
`node bin/harness.mjs dispatch --language-profile <name>` (see § Language profiles
below).
...
`node bin/harness.mjs dispatch --language-profile <name>`, which splices the
...
all live in `node bin/harness.mjs review-output --help`. The aggregator
...
`## Model audit`. **How:** run `node bin/harness.mjs review --help` for the full
...
Run `node bin/harness.mjs copilot-engage <pr-number>` to request the Copilot
...
codes live in `node bin/harness.mjs copilot-engage --help`. Key doctrine the
...
`node bin/harness.mjs install-hooks` installs an **opt-in** git `prepare-commit-msg`
...
- **Opt-in only.** `node bin/harness.mjs init` never installs the hook; it is written
