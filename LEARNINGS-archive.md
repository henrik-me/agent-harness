# Learnings & Decisions — Archive

These learning entries were moved out of [`LEARNINGS.md`](LEARNINGS.md) by CS65 (process-doc right-sizing). Every entry here has status `applied` or `obsolete` **and** a `date` earlier than `2026-06-01` — the aged, already-dispositioned backlog that no longer needs to sit in the always-loaded active log. Each moved entry is anchor-stable: `LEARNINGS.md` keeps a redirect **stub** at the same `#lrn-nnn` anchor pointing here, so every inbound `LEARNINGS.md#lrn-nnn` link and every bare `LRN-###` token still resolves unchanged. These entries are historical: **do not edit them** — file new learnings in `LEARNINGS.md` instead.
