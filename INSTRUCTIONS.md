...
### Repository claiming phases

- **CS01 bootstrap only:** the very first commit to `main` is a documented
  one-time exception. From commit 2 onward every change — including WORKBOARD
  claim/closeout — goes through a PR.
- **CS01–CS14 (discipline-enforced):** GitHub branch protection is not
  available on private free-tier repos (see [LRN-001](LEARNINGS.md#lrn-001)).
  Discipline + GPT-5.5 + user review enforce the policy during this phase.
...
Per-CS-loop review mapping for the self-host:

- Private phase (CS01–CS14): GPT-5.5 + user review. Copilot review optional.
- Public protected phase (current): GPT-5.6 Sol + required PR checks + one
  approving review by default; repository admins may use the explicit Ruleset
  bypass for owner override; workboard-only PRs use the validated bot path.

### Cross-repo issue-filing scope

**Scope clarification (CS55 / LRN-137):** the "agent does not file issues in
this repository" rule applies to the harness repo only. Cross-repo handoff
issues filed into OTHER repositories (e.g. `henrik-me/sub-invaders`) are
governed by Hard Rule § 6 in `.github/copilot-instructions.md` and the
`## Cross-repo procedures` section in `OPERATIONS.md`. In those repos, the
orchestrator MUST file an issue (rather than commit/push/PR directly) and is
expected to create exactly one tracking issue labeled `harness-orchestrator`
per cross-repo workstream.

### Rubber-duck fact-claim evidence

Pattern verified on PR #218: 3 substantive Copilot rounds caught 7 fact-claim
issues (R4 returned 0 findings) that the rubber-duck pre-review missed because
the review prompt did not require cross-surface verification.
...
- Knowledge-lives-in-the-repo doctrine — [LRN-153](LEARNINGS.md#lrn-153).
...
- Schema-first field access — [LRN-039](LEARNINGS.md#lrn-039).
- Briefing-preamble verbatim paste — [LRN-068](LEARNINGS.md#lrn-068).
- Per-file size check after parallel waves — [LRN-017](LEARNINGS.md#lrn-017).
- Pre-release audit gate — [LRN-101](LEARNINGS.md#lrn-101).
- `--file` flag, never infer target path — [LRN-032](LEARNINGS.md#lrn-032).
- `requireValue` flag-value parsing — [LRN-040](LEARNINGS.md#lrn-040).
- `--help` forwarding for subcommands — [LRN-030](LEARNINGS.md#lrn-030).
- Single resolved `--config` path — [LRN-038](LEARNINGS.md#lrn-038).
- `spawnSync` with `shell: true` on Windows — [LRN-029](LEARNINGS.md#lrn-029).
