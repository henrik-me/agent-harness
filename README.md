...
> **Status:** v0.4.0 shipped (CS39, 2026-05-13) — #145 enforcement-doctrine arc complete (CS35–CS38b). Adds `harness pr-evidence` (B1+A3+A4+A5+A6+A16 PR-time gates), the canonical PR template skeleton + sync migration, and the bundled `.github/workflows/pr-evidence-lint.yml` pre-merge enforcement. v0.3.1 (CS30, 2026-05-12) shipped SI bootstrap-feedback fixes + release pin guidance. v0.3.0 (CS29, 2026-05-11) shipped the BREAKING WORKBOARD shape change. Public-flip complete (CS15a). See [`CHANGELOG.md`](CHANGELOG.md) for the full delta and [`project/clickstops/done/done_cs01_bootstrap-repo/harness-cs-plan.md`](project/clickstops/done/done_cs01_bootstrap-repo/harness-cs-plan.md) for the roadmap. **Next: v0.5.0 arc** (CS40 review-output linter + CS41 `harness copilot-engage` CLI + agent-identity columns + `review_gates` default-flip — already on main in `[Unreleased]`; CS42 release-cut next).
...
```bash
# In a consumer repo:
npx -y github:henrik-me/agent-harness#v0.4.0 init
