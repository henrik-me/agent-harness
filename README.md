...
```bash
# In a consumer repo:
npx -y github:henrik-me/agent-harness#v0.16.0 init
# review the generated harness.config.json, then:
npx -y github:henrik-me/agent-harness#v0.16.0 sync
```
...
```bash
node agent-harness/bin/harness.mjs upgrade v0.16.0   # preview only
# review the change list, then bump harness.config.json "version" to v0.16.0 and:
