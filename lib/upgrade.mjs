 ...
 * consumer repo, printing the rendered diff + a migration summary — **without
 * applying anything**. Apply remains the explicit, separate `harness sync
 * --mode=apply` step.
 *
...
export function defaultFetchHarnessAtRef(ref, { repoUrl = DEFAULT_REPO_URL } = {}) {
  ...
  if (clone.status !== 0) {
    throw new UpgradeError(`git clone failed: ${(clone.stderr || '').trim()}`, 'EUPGRADE_CLONE');
  }
  const co = spawnSync('git', ['-C', dir, 'checkout', '--quiet', ref], { encoding: 'utf8' });
  if (co.status !== 0) {
    throw new UpgradeError(`git checkout "${ref}" failed: ${(co.stderr || '').trim()}`, 'EUPGRADE_CHECKOUT');
  }
  return dir;
}
