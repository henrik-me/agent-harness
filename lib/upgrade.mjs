...
import { makeTempDir, assertSafeRef } from './disposers.mjs';
...
export function defaultFetchHarnessAtRef(ref, { repoUrl = DEFAULT_REPO_URL } = {}) {
  // CS64b (C64b-2): validate the ref through the shared assertSafeRef guard
  // (lib/disposers.mjs) so the leading-dash / argv-injection rule lives in one
  // place; preserve the UpgradeError(EUPGRADE_BAD_REF) contract for callers.
  try {
    assertSafeRef(ref);
  } catch (e) {
    throw new UpgradeError(`invalid ref "${ref}": ${e.message}`, 'EUPGRADE_BAD_REF');
  }
  // CS64b (C64b-2): shared provenance-safe temp-dir disposer (paired allocation
  // + idempotent cleanup) rather than an inline mkdtemp + best-effort rmSync.
  const { path: dir, cleanup } = makeTempDir('harness-upgrade-');
  try {
...
export async function planUpgrade({
  ...
  // CS64b (C64b-2): shared assertSafeRef guard (after the empty-ref NO_REF check,
  // which assertSafeRef would otherwise collapse into a generic rejection).
  try {
    assertSafeRef(targetRef);
  } catch (e) {
    throw new UpgradeError(`invalid ref "${targetRef}": ${e.message}`, 'EUPGRADE_BAD_REF');
  }
