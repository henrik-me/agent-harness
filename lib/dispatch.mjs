 ...
 * that has not yet run `harness sync --apply-new` to adopt the managed
 ...
 *   2. else `<cwd>/OPERATIONS.md` — the legacy inline-fenced source (C86-4
 *      fallback: a pre-`sync --apply-new` consumer still ships it);
 *   3. else throw a clear `Error` naming both paths + the adoption command.
 *
...
export function resolvePreambleSource({ cwd } = {}) {
  ...
  if (preambleSourceReadable(legacy)) return legacy;
  throw new Error(
    `dispatch: no briefing-preamble source found under ${cwd}. Looked for:\n` +
      `  ${primary}\n` +
      `  ${legacy}\n` +
      `The managed ${PREAMBLE_SOURCE_BASENAME} ships with the harness; a consumer ` +
      `that has not yet adopted it must run \`harness sync --apply-new\` to ` +
      `materialize it.`
  );
}
