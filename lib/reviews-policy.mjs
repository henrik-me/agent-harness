 ...
 * imported by four harness linters: the three PR-side review-gate scripts that
 * `review-gates.yml` runs (`scripts/checks/check-review-log-evidence.mjs`,
 * `check-independence-invariant.mjs`, `check-copilot-review-attached.mjs`) plus
 * the `harness lint`-run `scripts/check-clickstop-implementer-not-reviewer.mjs`.
 * The dep-free requirement is driven by the first three: `review-gates.yml`
 * runs them from a bare `git clone` of the harness repo with **no `npm
 * install`**, so any transitive third-party import (e.g. AJV via
 * `lib/config-reader.mjs`) would crash them with `ERR_MODULE_NOT_FOUND`.
 * (`check-clickstop-implementer-not-reviewer.mjs` runs under `harness lint` with
 * full deps, but is kept on this dep-free module for consistency.) This module
 * is therefore deliberately separate from `lib/config-reader.mjs` (which pulls
 * in AJV for full-config validation) — resolving CS61 plan Q1: the
 * reviews-policy reader lives in its own dep-free module, NOT in
 * `config-reader.mjs`.
 *
...
export function loadReviewsPolicy({ cwd, configPath = null } = {}) {
  ...
  if (!existsSync(candidate)) {
    if (explicit) {
      throw new ReviewsConfigError(`--config file not found: ${candidate}`, {
        code: 'NOT_FOUND',
...
// Cache the review_gates.enforcement enum surface (schema-sourced, per LRN-039 /
// C61-2 schema-is-source-of-truth) so each read does not re-parse the schema.
let reviewGatesEnforcementEnumCache = null;

function getReviewGatesEnforcementEnum() {
  if (reviewGatesEnforcementEnumCache) return reviewGatesEnforcementEnumCache;
  const schema = JSON.parse(readFileSync(schemaPath, 'utf8'));
  reviewGatesEnforcementEnumCache =
    schema?.properties?.review_gates?.properties?.enforcement?.enum ?? [
      'human-approval',
      'required-check',
      'both',
    ];
  return reviewGatesEnforcementEnumCache;
}

/**
 * Read `review_gates.enforcement` with an explicit PRESENCE signal (CS109 /
 * ADR 0006 D1). Unlike {@link loadReviewsPolicy}, which materializes schema
 * defaults and therefore erases the absent-vs-set distinction, this reader
 * PRESERVES absence: the ruleset renderer keys off `present` so that an ABSENT
 * `review_gates.enforcement` leaves ruleset generation byte-for-byte unchanged
 * (structurally preventing a silent demotion of an existing consumer's required
 * gates). The field deliberately carries no schema `default`.
 *
 * Fail-closed (throws {@link ReviewsConfigError} with code `MALFORMED`) when
 * `review_gates` is present-but-not-an-object, or `review_gates.enforcement` is
 * present but not one of the schema enum values. A missing config file with no
 * explicit `configPath` resolves to `{ present: false }` (same posture as
 * `loadReviewsPolicy`); an explicit-but-missing `configPath` throws `NOT_FOUND`.
 *
 * @param {{cwd?: string, configPath?: string|null}} [opts]
 * @returns {{present: boolean, value: ('human-approval'|'required-check'|'both'|undefined)}}
 */
export function loadReviewGatesEnforcement({ cwd, configPath = null } = {}) {
  const baseCwd = cwd ?? process.cwd();
  const explicit = Boolean(configPath);
  const candidate = explicit
    ? path.resolve(baseCwd, configPath)
  ...
  try {
    ...
    // permission errors (EACCES/EPERM), which would misclassify an
    // unreadable-but-present config as "absent" (implicit) or NOT_FOUND
    // (explicit) instead of failing closed. Read directly and discriminate
    // err.code === 'ENOENT' (repo fs-error-handling convention; cf.
    // scripts/check-config-placeholders.mjs, scripts/check-clickstop.mjs).
    if (err.code === 'ENOENT') {
      if (explicit) {
        throw new ReviewsConfigError(`--config file not found: ${candidate}`, {
          code: 'NOT_FOUND',
  ...
  if (!hasOwnKey(config, 'review_gates')) {
    return { present: false, value: undefined };
  }
  const reviewGates = config.review_gates;
  if (!reviewGates || typeof reviewGates !== 'object' || Array.isArray(reviewGates)) {
    failReviews('review_gates must be an object when present', candidate, 'review_gates');
  }
  if (!hasOwnKey(reviewGates, 'enforcement')) {
    return { present: false, value: undefined };
  }
  const value = reviewGates.enforcement;
  const enumValues = getReviewGatesEnforcementEnum();
  if (typeof value !== 'string' || !enumValues.includes(value)) {
    failReviews(
      `review_gates.enforcement must be one of ${enumValues
        .map((v) => `"${v}"`)
        .join(', ')} when present`,
      candidate,
      'review_gates.enforcement'
    );
  }
