...
Options:
  ...
  --quiet         Suppress success output on stdout. Errors/warnings still
                  go to stderr.
...
/**
 * READ-ONLY. Walk `<gitDir>/refs/remotes/` and return every loose ref file
...
function stripPackedRefsLines(body, brokenRefPaths) {
  ...
  // Preserve a trailing newline only if the original had one and content remains.
  if (newlineCount === 0 || result === '') {
    return { body: result, stripped };
  }
  return { body: result, stripped };
}
 ...
 * @param {string} params.gitDir - Absolute path to the `.git` directory.
 * @param {object} [params.fs] - Filesystem module (injectable; default node fs).
 * @param {(args: string[]) => {status: number|null, stdout: string, stderr: string}} params.runGit
 * @returns {{deleted: string[], packedRefsStripped: number, fetch: {status: number|null, stdout: string, stderr: string}}}
 */
export function repairBrokenLooseRefs({ findings, gitDir, fs = nodeFs, runGit }) {
  ...
  if (fs.existsSync(packedRefsPath)) {
    const original = fs.readFileSync(packedRefsPath, 'utf8');
  ...
  const fetch = runGit(['fetch', 'origin', '--prune']);
...
export async function doctor(args, deps = {}) {
  ...
  let result;
  try {
    result = repairBrokenLooseRefs({ findings, gitDir, fs, runGit });
  } catch (e) {
