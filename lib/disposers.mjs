...
export function makeTempDir(prefix = 'harness-', { fs: fsImpl = fs } = {}) {
  ...
  // Defense-in-depth (CS64b Copilot review): the prefix must be a simple
  // basename so a path separator, '..', or absolute path can never make
  // mkdtemp (and the recursive cleanup) escape os.tmpdir().
  if (prefix.includes('/') || prefix.includes('\\') || prefix.includes('..') || path.isAbsolute(prefix)) {
    throw new Error(
      `makeTempDir prefix must be a simple basename (no path separators, '..', ` +
      `or absolute path), got ${JSON.stringify(prefix)}`
    );
  }
  // Capture the exact allocated path; cleanup removes only this, never a guess.
  const dir = fsImpl.mkdtempSync(path.join(os.tmpdir(), prefix));
  let removed = false;
  const cleanup = () => {
    if (removed) return;
    removed = true;
