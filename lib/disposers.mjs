...
export function makeTempDir(prefix = 'harness-', { fs: fsImpl = fs } = {}) {
  ...
  // Defense-in-depth (CS64b Copilot review + R9/R10): the prefix must be a
  // non-empty SIMPLE basename — no path separators, '.', '..', trailing
  // separator, or absolute path — so neither the mkdtemp allocation nor the
  // recursive cleanup can escape os.tmpdir(). `prefix !== path.basename(prefix)`
  // rejects every separator form (incl. a trailing sep or a leading-separator
  // single component that path.join would otherwise normalize into a child).
  if (
    prefix === '' ||
    prefix === '.' ||
    prefix === '..' ||
    ...
    throw new Error(
      `makeTempDir prefix must be a non-empty simple basename (no path separators, ` +
      `'.', '..', or absolute path); got ${JSON.stringify(prefix)}`
    );
  }
  const tmpRoot = os.tmpdir();
  const stem = path.join(tmpRoot, prefix);
  // Backstop: the mkdtemp template must sit directly inside os.tmpdir().
  if (path.dirname(stem) !== tmpRoot) {
    throw new Error(
      `makeTempDir prefix did not resolve directly under os.tmpdir(); got ${JSON.stringify(prefix)}`
    );
  }
