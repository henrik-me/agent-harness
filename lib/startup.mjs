 ...
 * instead of a cryptic `Template file not found`. It is exempt on the self-host
 * / pre-release sentinel and suppressible via `--skip-version-check`.
 *
 * Output:
...
export function runStartup({ runner, snapshot, agentId, opts = {} }) {
  ...
  // checks (so the stale-CLI-vs-pulled-config collision fails fast instead of
  // surfacing later as a cryptic Template-not-found). Additive only: the check
  // is present ONLY when the runner exposes readVersionInfo() (real disk runs +
  // opted-in tests), so existing stub-runner behaviour is unchanged.
