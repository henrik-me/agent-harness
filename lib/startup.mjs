/**
 * lib/startup.mjs — read-only session-bootstrap verb (CS64 C64-3).
 *
 * Mechanizes the INSTRUCTIONS.md § Session Start sanity sequence into a
 * single command. Read-only and advisory: it reports the state of the repo
 * but never mutates anything. The exit code is binary — non-zero ONLY on a
 * genuinely broken tree (the "main is green" gate per C64-3), so an
 * orchestrator that just wants to see "what's in flight?" can run it freely
 * without it ever wedging a claim.
 *
 * Checks run, in order:
 *   1. git fast-forward probe (`git pull --ff-only` if requested)  — advisory
 *   2. version pin (get-latest-first, vs harness.config.json)       — broken
 *   3. clean-worktree check                                         — advisory
 *   4. `node --test tests/*.test.mjs`                               — broken
 *   5. `harness lint --quiet`                                       — broken
 *   6. `harness sync --mode=check`                                  — broken
 *
 * The version-pin check (CS111 / C111-1..C111-3, issue #502) runs AFTER the
 * fast-forward probe so a pin advanced by `--pull-ff-only` is evaluated against
 * the PULLED config, and BEFORE the clean/tests/lint/sync checks so a stale-CLI
 * -vs-pulled-config collision fails fast with an actionable dual-version message
 * instead of a cryptic `Template file not found`. On a mismatch it SHORT-CIRCUITS
 * (the remaining checks are skipped). It is exempt on the self-host / pre-release
 * sentinel and suppressible via `--skip-version-check`.
 *
 * Output:
 *   - Agent ID + HEAD SHA (the "INSTRUCTIONS re-read @ <SHA>" line).
 *   - One status line per check (✓/⚠/✖).
 *   - The in-flight planned/active CS listing (delegates to lib/status.mjs).
 *
 * All git / shell / npm-test access is funnelled through an injectable
 * `runner` interface so unit tests can drive the pure logic without spawning
 * any process or touching the real worktree (R6/LRN-094 test hygiene).
 *
 * Zero runtime dependencies beyond Node 20+ stdlib + lib/status.mjs.
 *
 * @module lib/startup.mjs
 */

import { spawnSync } from 'node:child_process';
import { readFileSync } from 'node:fs';
import path from 'node:path';

import { getStatusSnapshotFromDisk, formatStatusReport } from './status.mjs';
import { evaluateVersionMatch, buildMismatchMessage } from './version-check.mjs';

/**
 * @typedef {object} CheckResult
 * @property {string}   name       - Human-readable label (e.g. 'git fast-forward').
 * @property {'pass'|'fail'|'skip'} status
 * @property {'broken'|'advisory'} severity
 * @property {string}   [message]  - One-line detail (always present on fail).
 */

/**
 * @typedef {object} StartupRunner
 * @property {() => string} gitHead
 * @property {() => boolean} gitWorktreeClean
 * @property {() => {ok: boolean, message: string}} [gitPullFfOnly]
 * @property {() => ({pkgVersion: string, configVersion: string, provenance: (object|null)}|null)} [readVersionInfo]
 * @property {() => {ok: boolean, summary: string}} runTests
 * @property {() => {ok: boolean, summary: string}} runLint
 * @property {() => {ok: boolean, summary: string}} runSyncCheck
 */

/**
 * @typedef {object} StartupResult
 * @property {string}          agentId
 * @property {string}          headSha
 * @property {CheckResult[]}   checks
 * @property {0|1}             exitCode  - 1 iff any 'broken'-severity check failed.
 * @property {object}          snapshot  - The lib/status snapshot (in-flight arc).
 */

/**
 * Reconstruct the startup args to echo in the corrective re-run command, so a
 * mismatch message tells the operator to re-run with the SAME flags they used
 * (e.g. `startup --pull-ff-only`). `--skip-version-check` is deliberately NOT
 * echoed — the re-run is meant to fix the mismatch, not suppress it.
 *
 * @param {{pullFfOnly?: boolean}} [opts]
 * @returns {string[]}
 */
function rerunStartupArgs(opts = {}) {
  const args = ['startup'];
  if (opts.pullFfOnly) args.push('--pull-ff-only');
  return args;
}

/**
 * Run the bootstrap sequence against an injected runner.
 *
 * @param {object} args
 * @param {StartupRunner} args.runner
 * @param {object}        args.snapshot - lib/status.mjs snapshot for the report tail.
 * @param {string}        args.agentId
 * @param {object}        [args.opts]
 * @param {boolean}       [args.opts.pullFfOnly]  - When true, run the git fast-forward probe.
 * @param {boolean}       [args.opts.skipVersionCheck] - When true, skip the get-latest-first version-pin preflight.
 * @returns {StartupResult}
 */
export function runStartup({ runner, snapshot, agentId, opts = {} }) {
  const checks = /** @type {CheckResult[]} */ ([]);
  const headSha = runner.gitHead();

  if (opts.pullFfOnly && typeof runner.gitPullFfOnly === 'function') {
    const r = runner.gitPullFfOnly();
    checks.push({
      name: 'git fast-forward probe',
      status: r.ok ? 'pass' : 'fail',
      severity: 'advisory',
      message: r.message,
    });
  }

  // ── Version-pin (get-latest-first) preflight (CS111 / C111-1..C111-3) ──────
  // Runs AFTER the fast-forward probe (so a pin advanced by --pull-ff-only is
  // evaluated against the PULLED config) and BEFORE the clean/tests/lint/sync
  // checks. On a real mismatch it SHORT-CIRCUITS (returns immediately, skipping
  // clean/tests/lint/sync) so the operator sees ONLY the actionable re-run
  // message rather than the cryptic Template-not-found noise those checks would
  // produce under the wrong CLI. Additive only: the check is present ONLY when
  // the runner exposes readVersionInfo() (real disk runs + opted-in tests), so
  // existing stub-runner behaviour is unchanged.
  if (typeof runner.readVersionInfo === 'function') {
    if (opts.skipVersionCheck) {
      checks.push({
        name: 'version pin (get-latest-first)',
        status: 'skip',
        severity: 'broken',
        message: 'skipped via --skip-version-check',
      });
    } else {
      const info = runner.readVersionInfo();
      // info === null → no harness.config.json (fresh checkout): skip, never fail.
      if (info === null || info === undefined) {
        checks.push({
          name: 'version pin (get-latest-first)',
          status: 'skip',
          severity: 'broken',
          message: 'no harness.config.json (fresh checkout) — version pin not evaluated',
        });
      } else {
        const verdict = evaluateVersionMatch({
          runningPkgVersion: info.pkgVersion,
          configVersion: info.configVersion,
          provenance: info.provenance,
        });
        if (verdict.outcome === 'mismatch') {
          checks.push({
            name: 'version pin (get-latest-first)',
            status: 'fail',
            severity: 'broken',
            message: buildMismatchMessage({ result: verdict, startupArgs: rerunStartupArgs(opts) }),
          });
          // Fail fast: a stale-CLI-vs-pulled-config mismatch means the remaining
          // checks (tests/lint/sync) would run under the WRONG CLI and surface the
          // very cryptic Template-not-found noise this preflight exists to preempt.
          // Short-circuit so the operator sees ONLY the actionable re-run message.
          return { agentId, headSha, checks, exitCode: 1, snapshot };
        } else if (verdict.outcome === 'match') {
          checks.push({
            name: 'version pin (get-latest-first)',
            status: 'pass',
            severity: 'broken',
            message: `running CLI matches pinned version ${verdict.configVersion}`,
          });
        } else {
          checks.push({
            name: 'version pin (get-latest-first)',
            status: 'skip',
            severity: 'broken',
            message: `exempt (${verdict.exemptReason})`,
          });
        }
      }
    }
  }

  const clean = runner.gitWorktreeClean();
  checks.push({
    name: 'clean worktree',
    status: clean ? 'pass' : 'fail',
    severity: 'advisory',
    message: clean ? '' : 'worktree has uncommitted changes (advisory — normal during work)',
  });

  const tests = runner.runTests();
  checks.push({
    name: 'node --test tests/*.test.mjs',
    status: tests.ok ? 'pass' : 'fail',
    severity: 'broken',
    message: tests.summary,
  });

  const lint = runner.runLint();
  checks.push({
    name: 'harness lint --quiet',
    status: lint.ok ? 'pass' : 'fail',
    severity: 'broken',
    message: lint.summary,
  });

  const sync = runner.runSyncCheck();
  checks.push({
    name: 'harness sync --mode=check',
    status: sync.ok ? 'pass' : 'fail',
    severity: 'broken',
    message: sync.summary,
  });

  const broken = checks.some((c) => c.status === 'fail' && c.severity === 'broken');
  return {
    agentId,
    headSha,
    checks,
    exitCode: broken ? 1 : 0,
    snapshot,
  };
}

/**
 * Parse a `node --test` (or `node --test --test-reporter=tap`) child-process
 * stdout/stderr blob and return the pass/fail counts.
 *
 * Node's default reporter is `spec` when stdout is a TTY (lines like
 * `ℹ pass 1272`) and `tap` otherwise (lines like `# pass 1272`). We accept
 * either form so the startup summary is accurate regardless of how the
 * child is spawned.
 *
 * @param {string} text - Combined stdout + stderr.
 * @returns {{ pass: number, fail: number }}
 */
export function parseNodeTestSummary(text) {
  // Anchor to line-start so test-name lines that happen to contain "ℹ pass N"
  // or "# pass N" inside their body don't pollute the count.
  const passMatch = /^(?:ℹ|#)\s+pass\s+(\d+)\s*$/m.exec(text || '');
  const failMatch = /^(?:ℹ|#)\s+fail\s+(\d+)\s*$/m.exec(text || '');
  return {
    pass: passMatch ? Number(passMatch[1]) : 0,
    fail: failMatch ? Number(failMatch[1]) : 0,
  };
}

/** Status glyph for a check. */
function glyph(c) {
  if (c.status === 'pass') return '✓';
  if (c.status === 'skip') return '–';
  return c.severity === 'broken' ? '✖' : '⚠';
}

/**
 * Render a startup result as a human-readable report ending in a trailing newline.
 *
 * @param {StartupResult} r
 * @returns {string}
 */
export function formatStartupReport(r) {
  const lines = [];
  lines.push(`harness startup — agent ${r.agentId}`);
  lines.push(`INSTRUCTIONS.md re-read complete @ ${r.headSha}`);
  lines.push('');
  lines.push('Bootstrap sanity check:');
  for (const c of r.checks) {
    const tag = c.severity === 'broken' ? '' : ' (advisory)';
    const detail = c.message ? ` — ${c.message}` : '';
    lines.push(`  ${glyph(c)} ${c.name}${tag}${detail}`);
  }
  lines.push('');
  lines.push(formatStatusReport(r.snapshot).trimEnd());
  return lines.join('\n') + '\n';
}

/**
 * Build a real runner that shells out to git/node/harness on disk. Used by
 * the CLI; tests inject a stub runner instead.
 *
 * @param {object} args
 * @param {string} args.cwd
 * @param {string} args.harnessBin - Absolute path to bin/harness.mjs (so the
 *   sub-invocation is anchored at the same install the caller is running).
 * @returns {StartupRunner}
 */
export function createDefaultRunner({ cwd, harnessBin }) {
  // shell: false everywhere — on Windows cmd.exe, args like `--format=%H`
  // would be subject to %VAR% expansion if we shell out (Copilot R12 finding).
  // spawnSync('git', ...) without shell still resolves PATH+PATHEXT on
  // Windows via Node's built-in lookup, so no shell is needed.
  const spawnOpts = { cwd, encoding: 'utf8', shell: false };

  const runGit = (args) => spawnSync('git', args, spawnOpts);
  const runNode = (args) => spawnSync(process.execPath, args, spawnOpts);
  const runHarness = (args) =>
    spawnSync(process.execPath, [harnessBin, ...args], spawnOpts);

  return {
    gitHead() {
      const r = runGit(['log', '-1', '--format=%H']);
      if (r.status !== 0) return '';
      return (r.stdout || '').trim();
    },
    gitWorktreeClean() {
      const r = runGit(['status', '--porcelain']);
      if (r.status !== 0) return false;
      return (r.stdout || '').trim().length === 0;
    },
    gitPullFfOnly() {
      const r = runGit(['pull', '--ff-only', 'origin', 'main']);
      const ok = r.status === 0;
      const out = ((r.stdout || '') + (r.stderr || '')).trim().split('\n').pop() || '';
      return { ok, message: ok ? 'fast-forward OK' : `pull failed: ${out}` };
    },
    readVersionInfo() {
      // Fail-soft everywhere: startup must never crash on the version probe.
      // Returns null when there is no consumer harness.config.json (e.g. a
      // fresh checkout), which the caller treats as "skip, don't fail".
      const readJson = (filePath) => {
        try {
          const raw = readFileSync(filePath, 'utf8').replace(/^\uFEFF/, '');
          return JSON.parse(raw);
        } catch {
          return null;
        }
      };
      const config = readJson(path.join(cwd, 'harness.config.json'));
      if (config === null) return null;
      // harnessBin is <harnessRepoRoot>/bin/harness.mjs → root is two dirs up.
      const harnessRoot = path.dirname(path.dirname(harnessBin));
      const pkg = readJson(path.join(harnessRoot, 'package.json'));
      const lock = readJson(path.join(cwd, '.harness-lock.json'));
      return {
        pkgVersion: pkg && typeof pkg.version === 'string' ? pkg.version : '',
        configVersion: typeof config.version === 'string' ? config.version : '',
        provenance: lock
          ? { harness_ref: lock.harness_ref, resolved_sha: lock.resolved_sha }
          : null,
      };
    },
    runTests() {
      const r = runNode(['--test', 'tests/*.test.mjs']);
      const text = (r.stdout || '') + (r.stderr || '');
      const { pass, fail } = parseNodeTestSummary(text);
      return {
        ok: r.status === 0,
        summary:
          r.status === 0
            ? `${pass} pass / ${fail} fail`
            : `node --test exited ${r.status}; ${pass} pass / ${fail} fail`,
      };
    },
    runLint() {
      const r = runHarness(['lint', '--quiet']);
      const text = (r.stdout || '') + (r.stderr || '');
      const totalMatch = /Total: (\d+) passed, (\d+) failed, (\d+) skipped/.exec(text);
      if (totalMatch) {
        const [, p, f, s] = totalMatch;
        return {
          ok: r.status === 0,
          summary: `${p} passed / ${f} failed / ${s} skipped`,
        };
      }
      return { ok: r.status === 0, summary: r.status === 0 ? 'lint passed' : `lint exited ${r.status}` };
    },
    runSyncCheck() {
      const r = runHarness(['sync', '--mode=check']);
      const text = (r.stdout || '') + (r.stderr || '');
      const lastLine = text.trim().split('\n').slice(-1)[0] || '';
      return {
        ok: r.status === 0,
        summary: r.status === 0 ? 'no drift detected' : `sync check failed: ${lastLine}`,
      };
    },
  };
}

/**
 * Convenience: run startup against the real disk + render the report.
 *
 * @param {object} args
 * @param {string} args.cwd
 * @param {string} args.harnessBin
 * @param {string} args.agentId
 * @param {object} [args.opts]
 * @returns {StartupResult}
 */
export function runStartupFromDisk({ cwd, harnessBin, agentId, opts = {} }) {
  const runner = createDefaultRunner({ cwd, harnessBin });
  const snapshot = getStatusSnapshotFromDisk({ cwd, agentId });
  return runStartup({ runner, snapshot, agentId, opts });
}
