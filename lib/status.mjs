...
/**
 * Normalise a raw agent-id for comparison. Returns null when the id is absent
 * or the literal "unknown" (id could not be derived — skip ownership checks
 * rather than emit false mismatches).
 *
 * @param {string|null|undefined} agentId
 * @returns {string|null}
 */
function knownAgentId(agentId) {
  const id = String(agentId ?? '').trim();
  return id && id !== 'unknown' ? id : null;
}

/**
 * Ownership suffix for an Active Work row whose line already prints
 * `owner=<owner>`. Exact trimmed-string equality — prefix-collision-safe
 * (`yoga-ae` !== `yoga-ae-c3`). Empty string when the agent-id is unknown or
 * the owner cell is empty / the em-dash placeholder.
 *
 * @param {string} owner
 * @param {string|null} you - Result of knownAgentId().
 * @returns {string}
 */
function activeWorkOwnershipSuffix(owner, you) {
  if (!you) return '';
  const o = String(owner ?? '').trim();
  if (!o || o === '—') return '';
  return o === you ? '  (you)' : `  (not you: ${you})`;
}

/**
 * Ownership suffix for an On-disk active listing, which carries no owner of its
 * own. Joins to the WORKBOARD Active Work rows by CS id (exact, upper-cased) to
 * source the owner. Flags a listing with no matching WORKBOARD row so a partial
 * claim state is visible.
 *
 * @param {string} cs - e.g. "CS10".
 * @param {Map<string,string>} ownerByCs - Upper-cased CS id → owner.
 * @param {string|null} you - Result of knownAgentId().
 * @returns {string}
 */
function onDiskOwnershipSuffix(cs, ownerByCs, you) {
  if (!you) return '';
  const key = String(cs ?? '').toUpperCase();
  if (!ownerByCs.has(key)) return '  (owner unknown — no WORKBOARD row)';
  const o = String(ownerByCs.get(key) ?? '').trim();
  if (!o || o === '—') return '';
  return o === you ? '  (you)' : `  (owner ${o} — not you: ${you})`;
}
 ...
 *
 *   Active Work (N):
 *     <CS-Task ID>: <Title> [<State>] owner=<Owner> branch=<Branch> updated=<date>[  (you) | (not you: <id>)]
 *     ...
 *
 *   On-disk active (N):
 *     <CS> — <slug> [dir|flat]  → <entry>[  (you) | (owner <owner> — not you: <id>)]
 *     ...
 *
 *   Planned queue (N):
 *     <CS> — <slug>
 *     ...
 *
 * When the agent-id is known, each Active Work / On-disk active row is annotated
 * with its ownership relative to the current agent so a resuming orchestrator in
 * a concurrent same-machine clone (e.g. `<id>-c<N>`) cannot mistake another
 * orchestrator's active CS for its own (#417). Comparison is exact full-id
 * equality — never a prefix match. Empty sections render a single "(none)" line.
 *
...
export function formatStatusReport(s) {
  ...
  const you = knownAgentId(s.agentId);
  lines.push(`harness status — agent ${id}`);
  lines.push('');

  // CS id → owner from the WORKBOARD Active Work rows, so On-disk listings
  // (which carry no owner) can be annotated with ownership too.
  const ownerByCs = new Map();
  for (const r of s.activeWorkRows) {
    ownerByCs.set(String(r.csTaskId ?? '').toUpperCase(), r.owner);
  }
  ...
  if (s.activeWorkRows.length === 0) {
    ...
    for (const r of s.activeWorkRows) {
      lines.push(
        `  ${r.csTaskId}: ${r.title} [${r.state}] owner=${r.owner} branch=${r.branch} updated=${r.lastUpdated}${activeWorkOwnershipSuffix(r.owner, you)}`
      );
    }
  }
  lines.push('');
  ...
  if (s.activeListings.length === 0) {
    ...
    for (const a of s.activeListings) {
      ...
      lines.push(`  ${a.cs} — ${a.slug} [${form}] → ${a.entry}${onDiskOwnershipSuffix(a.cs, ownerByCs, you)}`);
    }
  }
  lines.push('');
