 ...
 * @param {string} csId
 * @returns {{exists: boolean, missingFile?: boolean}}
 */
export function activeWorkRowExists(workboardPath, csId) {
  ...
  // R2 reviewer (gpt-5.5): plain startsWith(target) incorrectly matches
  // sibling CS ids like CS64b when looking for CS64. Match exact id or the
  // sub-task cell form `<target>-T<n>` (e.g. "CS64-T1"), but never the
  // suffix-id form (e.g. "CS64b").
  return {
    exists: rows.some((r) => {
      const cell = String(r.cs || '').toUpperCase();
      return cell === target || cell.startsWith(`${target}-`);
    }),
  };
}
