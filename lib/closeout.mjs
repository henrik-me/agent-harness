...
export function findActiveClickstop({ activeDir, csId, readdir, isDirectory }) {
  ...
  const flatRe = new RegExp(`^active_cs${num}_([a-z0-9-]+)\\.md$`);
  const dirRe = new RegExp(`^active_cs${num}_([a-z0-9-]+)$`);
...
export function applyCloseoutPlan({ plan, runner }) {
  ...
  const contextRel = path.relative(path.dirname(plan.workboardPath), plan.contextPath);
  const contextChanged = changed.some((f) => {
    const norm = f.replace(/\\/g, '/');
    return norm === contextRel.replace(/\\/g, '/') || norm.endsWith('CONTEXT.md');
  });
...
export function runCloseoutFromDisk({ cwd, csId, apply = false }) {
  ...
  const doneDir = path.join(cwd, 'project', 'clickstops', 'done');
