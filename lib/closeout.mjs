...
export function activeWorkRowExists(workboardPath, csId) {
  ...
  // parseActiveWorkRows from lib/claim.mjs returns rows shaped {cs, title, ...};
  // accept prefix-match so "CS64" matches "CS64" or "CS64-T1" cell text.
  return { exists: rows.some((r) => String(r.cs || '').toUpperCase().startsWith(target)) };
}
