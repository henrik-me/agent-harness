...
export function applyCloseoutPlan({ plan, runner }) {
  ...
  // and other nested copies, which can falsely satisfy the gate.
  const changed = runner.changedFiles();
  const contextRel = path.relative(path.dirname(plan.workboardPath), plan.contextPath).replace(/\\/g, '/');
  const contextChanged = changed.some((f) => f.replace(/\\/g, '/') === contextRel);
