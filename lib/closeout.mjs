...
export function findDoneByCsId({ doneDir, csId, readdir, isDirectory }) {
  ...
  for (const name of entries) {
    ...
    if (dir && isDirectory(path.join(doneDir, name))) {
      matches.push({
        csFilePath: path.join(doneDir, name, `${name}.md`),
        filename: `${name}.md`,
        cs: csId,
        slug: dir[1],
        directoryForm: true,
...
export function runCloseoutFromDisk({ cwd, csId, apply = false }) {
  ...
  if (!found.ok) {
    // Idempotency check (C64-5: close-out is idempotent). If the active→done
    // rename has already happened (e.g. after the close-out PR merged),
    // re-running `harness close-out <NN>` is a clean no-op rather than a
    // "no active <CS>" error. CS64 PVI gate (post-merge) identified this gap.
