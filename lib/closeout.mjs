...
/**
 * @typedef {object} DoneListing
 * @property {string}  csFilePath      - Absolute path of the done .md file.
 * @property {string}  filename
 * @property {string}  cs              - Canonical CS id.
 * @property {string}  slug
 * @property {boolean} directoryForm
 ...
 * @param {(p: string) => string[]} args.readdir
 * @param {(p: string) => boolean} args.isDirectory
 * @returns {{ok: true, listing: DoneListing} | {ok: false, error: string}}
 */
...
export function formatApplyReport({ plan, applied }) {
  ...
  if (applied.prReady) {
    ...
    lines.push('Open the workboard-only close-out PR with the label at creation:');
    lines.push('  gh pr create --base main --label workboard-only --title "chore(workboard): close out CS<NN>..." --body-file ...');
    lines.push('  \u26a0 Use --label workboard-only in gh pr create itself \u2014 do NOT add it post-hoc via');
    lines.push('  gh pr edit --add-label (a separate "labeled" event creates a race, per CS71).');
    lines.push('  Since CS71, evidence gates are also path-derived (green without the label); the label is');
    lines.push('  still required for workboard-auto-approve.yml to auto-merge.');
  } else {
...
export function activeWorkRowExists(workboardPath, csId) {
  // Copilot reviewer on PR #299 round 8 (post-77e5a6d): fs.existsSync()
  // returns false on errors (including permission errors), not only when
  // the file is genuinely absent. Gating the readFileSync try/catch behind
  // existsSync therefore misclassifies an unreadable-but-present WORKBOARD
  // as missingFile:true and silently no-ops, masking the I/O error we
  // explicitly want to surface. Attempt the read directly and discriminate
  // ENOENT (genuinely missing — fresh checkout) from other errors (real
  // I/O failure — must surface).
  let md;
  try {
    md = readFileSync(workboardPath, 'utf8');
  } catch (e) {
    if (e && e.code === 'ENOENT') {
      return { exists: false, missingFile: true };
    }
 ...
 *
 * Return shapes:
 *   - Normal close-out flow: {ok, errors?, plan?, preflight?, apply?}
 *   - Already-closed-out idempotent no-op (C64-5): {ok: true,
 *     alreadyClosedOut: true, message, doneListing}. The CLI
 *     (bin/harness.mjs cmdCloseOut) branches on `alreadyClosedOut` and
 *     prints `message` instead of running the normal close-out flow.
 *
 * @param {object} args
 * @param {string} args.cwd
 * @param {string} args.csId
 * @param {boolean} args.apply
 * @returns {{ok: boolean, errors?: string[], plan?: CloseoutPlan, preflight?: PreflightResult, apply?: ApplyCloseoutResult, alreadyClosedOut?: boolean, message?: string, doneListing?: DoneListing}}
 */
export function runCloseoutFromDisk({ cwd, csId, apply = false }) {
  ...
  // Copilot reviewer on PR #299 round 9 (post-ecdea10, symmetric to the
  // claim-side fix): fs.existsSync() returns false on access errors as
  // well as ENOENT, so gating readdirSync behind existsSync silently
  // treats an unreadable active/ or done/ directory as empty and yields
  // a misleading "no active <CS>" — which would then drive the
  // idempotency fall-through and mask the real I/O problem. Attempt
  // readdir directly and discriminate ENOENT (genuinely missing — return
  // []) from any other error code (re-throw so findActiveClickstop /
  // findDoneByCsId catch and surface as "cannot read <dir>: <msg>").
  const readdirSafe = (p) => {
    try {
