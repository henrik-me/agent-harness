 ...
 * Algorithm:
 *   1. Lowercase.
 *   2. Strip characters that are not word-chars (letters, digits, underscore),
 *      whitespace, or hyphen.
 *   3. Convert underscores and runs of whitespace to hyphens.
 *   4. Collapse runs of hyphens to one.
 *   5. Trim leading/trailing hyphens.
 *
 * Idempotent on already-slugged text. Mirrors the rule that GitHub applies
 * to markdown heading anchors.
 *
 * @param {string} text - Heading text (without the leading `#` markers).
 * @returns {string} GitHub-flavoured anchor slug.
 */
...
/**
 * Collect every ATX heading (`#` .. `######`) in a markdown document.
 *
 * BOM is stripped and CRLF/CR are normalized to LF before scanning. Headings
 * appearing inside fenced code blocks are NOT excluded — callers needing
 * code-fence awareness should filter the result.
 *
 * @param {string} markdownText - Raw markdown file content.
 * @returns {Array<{level: number, text: string, line: number, anchor: string}>}
 *   One entry per heading, in document order. `line` is 1-indexed.
 */
export function collectHeadings(markdownText) {
  const text = normalize(markdownText);
  const lines = text.split('\n');
  const headings = [];
  for (let i = 0; i < lines.length; i++) {
...
/**
 * Collect only level-2 (H2) headings from a markdown document.
 *
 * Convenience filter over {@link collectHeadings} that omits the redundant
 * `level` field. Headings of level 1 or 3..6 are skipped.
 *
 * @param {string} markdownText - Raw markdown file content.
 * @returns {Array<{text: string, line: number, anchor: string}>}
 *   One entry per H2 heading, in document order. `line` is 1-indexed.
 */
export function collectH2Headings(markdownText) {
  return collectHeadings(markdownText)
    .filter((h) => h.level === 2)
    .map(({ text, line, anchor }) => ({ text, line, anchor }));
}

/**
 * Extract the markdown body that lives under an H2 with the given anchor,
 * up to but not including the next H1 or H2 (or end-of-file).
 *
 * Body is line-joined with LF. Returns `''` when no H2 with that anchor is
 * found. When multiple H2s share the same anchor, the first is used. Deeper
 * headings (H3..H6) inside the body do NOT terminate the section — only H1
 * and H2 do.
 *
 * @param {string} markdownText - Raw markdown file content.
 * @param {string} h2Anchor    - Anchor slug to match (typically produced by
 *   {@link headingAnchor}).
 * @returns {string} Section body, or `''` if anchor not found.
 */
export function extractSectionBody(markdownText, h2Anchor) {
  const text = normalize(markdownText);
  const lines = text.split('\n');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (m && headingAnchor(m[1].trim()) === h2Anchor) {
      startIdx = i;
      break;
    }
  }
