...
export function formatClaimPlan(plan) {
  ...
  lines.push('');
  lines.push('After applying and committing, open the workboard-only claim PR with the label at creation:');
  lines.push('  gh pr create --base main --label workboard-only --title "chore(workboard): claim CS<NN>..." --body-file ...');
 ...
 * @param {(args: {cwd: string}) => {worktreeClean: () => boolean}} [args.runnerFactory]
 *   - Test seam for the git runner used by the `--takeover --apply` clean-worktree
 *   guard; defaults to `createDefaultRunner`.
 ...
 */
export function runClaimFromDisk({ cwd, csId, agentId, harnessBin, apply = false, skipHarvest = false, takeover = false, runnerFactory = createDefaultRunner }) {
  ...
  if (alreadyActive.ok) {
    ...
    if (ownerMismatch) {
      ...
      // Copilot (#425 R2): the takeover mutation runs in the pre-git
      // idempotency region (before the worktree-clean / on-main preflights), so
      // guard it explicitly — refuse to reassign on a dirty worktree so the
      // ownership change lands isolated and committable. Any branch is fine
      // (the message directs the user to a takeover branch); only cleanliness
      // matters for isolating the one-line WORKBOARD edit.
      const takeoverRunner = runnerFactory({ cwd });
      if (!takeoverRunner.worktreeClean()) {
        return {
          ok: false,
          errors: [
            `${normalized} takeover requires a clean worktree so the WORKBOARD Owner reassignment lands isolated — commit or stash your changes first, then re-run 'harness claim ${normalized} --takeover --apply'.`,
          ],
        };
      }
