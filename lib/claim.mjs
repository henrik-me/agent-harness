 ...
 *
 *   1. Preflights:  clean worktree, exactly-one matching planned CS file,
 *                   valid slug, claim branch absent, no existing Active row
 *                   for this orchestrator, no other CS already in flight.
 ...
 * moment of write, so a sibling clone that lands its own claim between the
 * preflight and the apply does not get silently overwritten — the diff is
 * applied to the *current* file, not a stale snapshot.
 *
...
export function findPlannedClickstop({ plannedDir, csId, readdir, isDirectory }) {
  ...
  const flatRe = new RegExp(`^planned_cs${num}_([a-z0-9-]+)\\.md$`);
  const dirRe = new RegExp(`^planned_cs${num}_([a-z0-9-]+)$`);
  ...
  for (const name of entries) {
    ...
    if (dirMatch && isDirectory(path.join(plannedDir, name))) {
      matches.push({
        entryPath: path.join(plannedDir, name, `${name}.md`),
        filename: `${name}.md`,
        cs: csId,
        slug: dirMatch[1],
        directoryForm: true,
 ...
 *
 * Returned shape mirrors lib/status.mjs parseActiveWorkRows() — callers
 * already use that vocabulary.
 *
...
export function planClaim({
  ...
  if (rows.some((r) => r.owner === agentId && r.state.includes('Active') && r.cs !== csId)) {
    errors.push(
      `orchestrator ${agentId} already owns an Active CS row in WORKBOARD (${
        rows.find((r) => r.owner === agentId).cs
      })`
    );
  }
...
export function formatClaimPlan(plan) {
  ...
  lines.push(
    `  2. git mv ${path.relative(process.cwd(), plan.sourcePath)} ${path.relative(
      process.cwd(),
      plan.destPath
    )}`
  );
...
export function applyClaimPlan({ plan, runner }) {
  ...
  // R3 race-aware re-read.
  const fresh = runner.readFile(plan.workboardPath);
  const { md, mutated } = insertActiveWorkRow({ workboardMd: fresh, row: plan.workboardRow });
...
export function runClaimFromDisk({ cwd, csId, agentId, harnessBin, apply = false, skipHarvest = false }) {
  ...
  const workboardMd = readFileSync(workboardPath, 'utf8');
  ...
  runner.createBranch(planned.plan.branch.replace(/\/content$/, '/claim'));
