 ...
 * @returns {{ok: boolean, errors?: string[], plan?: ClaimPlan, apply?: ApplyResult, harvest?: {ok: boolean, output: string}, alreadyClaimed?: boolean, message?: string, activeListing?: ActiveListing}}
 */
  ...
  if (alreadyActive.ok) {
    ...
    if (existsSync(workboardPath)) {
      try {
        ...
        // Copilot reviewer on PR #299 round 3 (symmetric fix on claim side):
        // distinguish "missing file" (fresh checkout — legitimately no
        // WORKBOARD on disk, OK to no-op) from "exists but unreadable"
        // (real I/O error — must surface, cannot verify consistency).
