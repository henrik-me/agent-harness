...
export async function runReview({
  ...
  if (dryRun) {
    return {
      ...
      round: round || 'next',
      actions: describePlannedActions({ rubberDuckOnly, copilotOnly, noPoll, config }),
      exitCode: 0,
    };
  }
...
function describePlannedActions({ rubberDuckOnly, copilotOnly, noPoll, config }) {
  const actions = [];
  if (!copilotOnly) actions.push(`compose manual rubber-duck prompt for ${config.rubber_duck_model}`);
  if (!rubberDuckOnly) actions.push(`trigger Copilot via ${config.copilot_trigger}`);
