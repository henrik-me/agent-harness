...
export function applyTemplating(content, variables, opts = {}) {
  ...
  processed = processed.replace(pattern, (match, key) => {
    // CS03b R1 review fix: a custom placeholderPattern with no first capture
    // group produces an undefined key. Guard so the user gets a clean
    // ETPL_BAD_PATTERN instead of mysteriously seeing `{{undefined}}` in
    // strict-mode error messages.
    if (typeof key !== 'string') {
      throw new TemplatingError(
        'placeholderPattern must include a first capture group for the placeholder key',
        'ETPL_BAD_PATTERN'
      );
    }
