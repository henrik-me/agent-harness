...
/**
 * Compute the ref the user should pin on re-run. When the config version
 * already carries a `v`/`V` prefix, the prefix is normalized to a lowercase
 * `v` (release tags in this repo are lowercase `v<semver>` and git refs are
 * case-sensitive, so echoing an uppercase `#V0.17.0` could 404); a 40-hex SHA
 * is returned verbatim; otherwise the normalized SemVer is `v`-prefixed (so the
 * re-run reads `#v0.17.0` / `#<sha>`).
 *
 * @param {*} configVersion
 * @returns {string}
 */
...
export function evaluateVersionMatch({ runningPkgVersion, configVersion, provenance } = {}) {
  ...
  // Self-host / pre-release sentinel (0.0.0, 0.0.0-pre, 0.0.0-<any>). Checked on
  // the NORMALIZED form so a `v`-prefixed sentinel (e.g. `v0.0.0-pre`, by
  // analogy with release tags) is also exempt rather than false-failing.
  if (/^0\.0\.0(?:$|[-+])/.test(normalizeVersion(cfg))) return exempt('sentinel');
