 ...
 *     via `git tag -a` + `git push origin` (OPERATIONS.md § Release process
 *     step 9) and the GitHub Release on it via `gh release create --verify-tag`
 *     (release-only, no `--target`), then file consumer notifications by reusing
 *     `openIssue` from `lib/cross-repo.mjs` (issue-only, Hard Rule § 6 / C67-4). Idempotent/resumable
 ...
 * Release on that already-existing tag via `gh release create v<version> --verify-tag`
 * (release-only: `--target` is NOT used; `--verify-tag` makes gh abort rather
 * than auto-create a lightweight tag if the pushed ref is absent).
