 ...
 *     files-at-SHA that do not carry the version), then create the ANNOTATED tag
 *     (`git tag -a v<x.y.z> <sha> -m "Release v<x.y.z>"` + `git push origin
 *     v<x.y.z>`, OPERATIONS.md § Release process step 9) and the GitHub Release on
 *     it via `gh release create v<x.y.z> --verify-tag` (release-only, no
 *     `--target`), then file consumer notifications by reusing `openIssue` from
 *     `lib/cross-repo.mjs` (issue-only, Hard Rule § 6 / C67-4). Idempotent/resumable
 *     (R7): a pre-existing tag+release pointing at the intended sha+version is
 *     skipped and only the consumer notifications are retried.
 *
 ...
 *
 * @returns {Array<{path:string, action:string, summary:string, oldContent:string, newContent:string}>}
 */
...
/**
 * Compute a compact unified-style line diff between two texts, dependency-free
 * (LCS backtrack — the release files are small, so O(n·m) is fine). Returns
 * lines prefixed with ' ' (context), '-' (removed) or '+' (added); unchanged
 * regions far from any edit are collapsed to a ` … ` marker.
 *
 ...
 * @returns {string[]}
 */
function unifiedDiffLines(oldText, newText, context = 2) {
  // Split into logical lines, dropping the terminal '' that a trailing '\n'
  // produces — so a normal file ending in a newline doesn't render a phantom
  // blank context line.
  const toLines = (t) => {
  ...
  // No differences → render no diff (avoids emitting a lone ` …` gap marker).
...
export function formatReleasePlan(plan) {
  ...
  if (!plan.changes || plan.changes.length === 0) {
    ...
    for (const c of plan.changes) {
      ...
      if (c.oldContent != null && c.newContent != null) {
        const diff = unifiedDiffLines(c.oldContent, c.newContent);
        if (diff.length > 0) {
          lines.push(`    --- a/${c.path}`);
          lines.push(`    +++ b/${c.path}`);
 ...
 * `main`, then (when `apply`) creates an ANNOTATED tag matching OPERATIONS.md
 * § Release process step 9 — `git tag -a v<version> <sha> -m "Release v<version>"`
 * then `git push origin v<version>` (via the `runGit` seam) — and the GitHub
 * Release on that already-existing tag via `gh release create v<version>
 * --verify-tag` (release-only: `--target` is NOT used; `--verify-tag` makes gh
 * abort rather than auto-create a lightweight tag if the pushed ref is absent).
 * Consumer notifications are filed via `openIssue` (issue-only, Hard Rule § 6).
 *
 * Idempotent/resumable (R7): a pre-existing REMOTE tag+release pointing at the
 * intended sha+version is skipped and only the consumer notifications are
 * retried. A tag pointing at a DIFFERENT sha is a hard error
 * (`ERELEASE_TAG_EXISTS`). When the remote tag is absent, the LOCAL tag is
 * consulted first: a prior run's `git push` may have failed leaving a local-only
 * tag, so a local tag at `<sha>` resumes by re-pushing (no `git tag -a` re-run),
 * a local tag at a different sha is `ERELEASE_TAG_EXISTS`, and a failed push
 * intentionally leaves the local tag in place for that resume path.
 *
...
export function publishRelease({ version, sha, cwd, repo, pr = null, consumers = [], apply = false, draft = true, seams = {} }) {
  ...
  // at our sha is the resumable case. Request BOTH the exact ref AND the peeled
  // `^{}` pattern — an exact single-refspec `ls-remote` does NOT auto-emit the
  // peeled commit line for an ANNOTATED tag, so without the second pattern
  // `parseLsRemoteSha` would see only the tag-OBJECT sha and mis-compare it to
  // the release commit (false ERELEASE_TAG_EXISTS). A lightweight tag has no
  // peel, so the second pattern simply matches nothing.
  const lsr = s.runGit(['ls-remote', '--tags', 'origin', `refs/tags/${tag}`, `refs/tags/${tag}^{}`]);
  if (lsr.status !== 0) {
    throw new ReleaseError(
  ...
  // Step 1 — annotated tag, then push (OPERATIONS.md § Release process step 9),
  // only when the REMOTE tag is absent. `gh release create --target` would make
  // a LIGHTWEIGHT tag; `git tag -a` makes the first-class annotated tag object
  // the documented process (and `parseLsRemoteSha`) expects.
  if (!tagPointsAtSha) {
    // A prior run's `git push` may have failed, leaving a LOCAL-only tag. Consult
    // it first so we resume by re-pushing rather than re-running `git tag -a`
    // (which would fail on the existing local tag).
    const localTagRes = s.runGit(['rev-parse', '-q', '--verify', `refs/tags/${tag}^{commit}`]);
    // `git rev-parse -q --verify` exits 1 when the ref is absent; any OTHER
    // non-zero status (e.g. 128 for a repo/cwd problem) is a real git failure
    // that must not be silently read as "no local tag".
    if (localTagRes.status !== 0 && localTagRes.status !== 1) {
      throw new ReleaseError(`git rev-parse for local tag ${tag} failed: ${failDetail(localTagRes)}`, 'ERELEASE_PUBLISH');
    }
    const localTagSha = localTagRes.status === 0 ? (localTagRes.stdout || '').trim() : null;
    if (localTagSha && localTagSha !== sha) {
      throw new ReleaseError(`tag ${tag} already exists locally at ${localTagSha}, not the requested ${sha}`, 'ERELEASE_TAG_EXISTS');
    }
    if (!localTagSha) {
      const tagRes = s.runGit(['tag', '-a', tag, sha, '-m', `Release ${tag}`]);
      if (tagRes.status !== 0) {
        throw new ReleaseError(`git tag -a ${tag} failed: ${failDetail(tagRes)}`, 'ERELEASE_PUBLISH');
      }
    }
    // Push the new (or resumed local-only) tag. On failure LEAVE the local tag in
    // place so a rerun's local-tag-at-<sha> detection above resumes by re-pushing.
    const pushRes = s.runGit(['push', 'origin', tag]);
    if (pushRes.status !== 0) {
      throw new ReleaseError(`git push origin ${tag} failed: ${failDetail(pushRes)}`, 'ERELEASE_PUBLISH');
    }
    result.tagCreated = true;
  }

  // Step 2 — the GitHub Release on the now-existing annotated tag. Release-only:
  // `--target` is gone (the tag already exists) and `--verify-tag` makes gh abort
  // rather than auto-create a LIGHTWEIGHT tag if the pushed ref is somehow absent.
  ...
  if (tagPointsAtSha && releaseExists) {
    ...
    const createArgs = ['release', 'create', tag, '--verify-tag', '--title', tag, '--notes', notes, ...repoArgs];
