...
function validateConfigSchema(config, harnessRepoPath) {
  ...
  if (typeof canonComposedOverrides === 'object' && !Array.isArray(canonComposedOverrides)) {
    canonComposedOverrides = Object.fromEntries(
      Object.entries(canonComposedOverrides).map(([k, v]) => [canonicalizeTargetPath(k), v])
    );
  }
  ...
  if (canonLocalBlocks != null && typeof canonLocalBlocks === 'object' && !Array.isArray(canonLocalBlocks)) {
    canonLocalBlocks = Object.fromEntries(
      Object.entries(canonLocalBlocks).map(([k, v]) => [canonicalizeTargetPath(k), v])
    );
  }
