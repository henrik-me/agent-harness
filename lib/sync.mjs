...
import { formatTemplateMissingVersionHint } from './version-check.mjs';
...
/**
 * Build the CS111 / C111-5 version-mismatch hint appended to an
 * `ESYNC_MISSING_TEMPLATE` error. Reads the running harness `pkg.version` from
 * `harnessRepoPath` (fail-soft) and pairs it with the consumer `config.version`
 * so the render-time error self-diagnoses a likely stale-CLI-vs-newer-config
 * collision. Never throws — a version that cannot be read degrades to 'unknown'
 * (or, if neither is available, to an empty hint), so the crash-path always
 * still throws the original error.
 *
 * @param {string} harnessRepoPath
 * @param {{version?: string}|null} config
 * @returns {string} A leading-space-prefixed hint, or '' when no version is known.
 */
function missingTemplateVersionHint(harnessRepoPath, config) {
  try {
    let pkgVersion = '';
    try {
      const raw = readFileSync(path.join(harnessRepoPath, 'package.json'), 'utf8').replace(/^\uFEFF/, '');
      const pkg = JSON.parse(raw);
      pkgVersion = typeof pkg.version === 'string' ? pkg.version : '';
    } catch {
      // Fail-soft: running pkg.version unknown — the hint degrades gracefully.
    }
    return formatTemplateMissingVersionHint({
      runningPkgVersion: pkgVersion,
      configVersion: config && typeof config.version === 'string' ? config.version : '',
    });
  } catch {
    return '';
  }
    ...
    if (templateRaw === null) {
      ...
      throw new SyncError(
        `Template file not found: ${templatePath} (required for ${fileClass} target "${target}")` +
          missingTemplateVersionHint(harnessRepoPath, config),
