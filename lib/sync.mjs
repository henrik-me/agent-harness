...
import { readLock, writeLock, validateLockObject } from './lock.mjs';
...
function validateConfigSchema(config, harnessRepoPath) {
  ...
  // Normalize paths before dedup: reject absolute/rooted/traversal, collapse './'.
  const seen = new Set();
  for (const target of [
    ...
    // Reject absolute paths (Unix-style /, Windows drive letters, rooted backslash, UNC).
    // Check BEFORE backslash normalization so rooted backslash forms are caught.
    if (/^\//.test(target) || /^\\/.test(target) || /^[A-Za-z]:/.test(target)) {
      throw new SyncError(
        `Invalid target path "${target}": absolute / rooted paths are not allowed (Unix-/, Windows drive, rooted backslash, or UNC).`,
        'EBADCONFIG_INVALID_PATH'
      );
    }
    // Canonicalize: backslash → forward slash, collapse './' segments, strip redundant slashes.
    const normalized = target
      .replace(/\\/g, '/')
      .split('/')
      .filter((seg, i, arr) => seg !== '' && seg !== '.' || (seg === '' && i === arr.length - 1))
      .filter(seg => seg !== '.')
      .join('/');
    // Reject paths with parent-directory traversal segments (after canonicalization).
    if (normalized.split('/').some(seg => seg === '..')) {
      throw new SyncError(
        `Invalid target path "${target}": paths must not contain ".." segments.`,
        'EBADCONFIG_INVALID_PATH'
      );
    }
    if (seen.has(normalized)) {
      throw new SyncError(
        `Duplicate target path "${target}" (canonical "${normalized}") appears in more than one file class ` +
