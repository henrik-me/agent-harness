...
function validateConfigSchema(config, harnessRepoPath) {
  ...
  // Normalize paths before dedup: reject absolute/traversal, normalize backslash → slash.
  const seen = new Set();
  for (const target of [
    ...
    // Reject absolute paths (Unix-style / and Windows drive letters).
    if (/^\//.test(target) || /^[A-Za-z]:/.test(target)) {
      throw new SyncError(
        `Invalid target path "${target}": absolute paths are not allowed.`,
        'EBADCONFIG_INVALID_PATH'
      );
    }
    // Reject paths with parent-directory traversal segments.
