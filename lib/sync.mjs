 ...
 * `composed.overrides` keys, and `local_blocks` keys.
 *
...
function validateConfigSchema(config, harnessRepoPath) {
  ...
  // ── Canonicalize local_blocks keys ────────────────────────────────────────
  let canonLocalBlocks = config.local_blocks;
  if (canonLocalBlocks != null && typeof canonLocalBlocks === 'object' && !Array.isArray(canonLocalBlocks)) {
    // Same Map-based collision detection (per GPT-5.5 review #6 blocking #1).
    const entries = new Map();
    for (const [k, v] of Object.entries(canonLocalBlocks)) {
      const canonK = canonicalizeTargetPath(k);
      if (entries.has(canonK)) {
        throw new SyncError(
          `Duplicate canonical key "${canonK}" in local_blocks ` +
          `(raw keys collide after canonicalization). Each file may have at most one local_blocks entry.`,
          'EBADCONFIG_DUP_PATH'
        );
      }
      entries.set(canonK, v);
    }
    canonLocalBlocks = Object.fromEntries(entries);
  }
  ...
  if (canonLocalBlocks != null) {
    canonical.local_blocks = canonLocalBlocks;
  }
