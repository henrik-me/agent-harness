...
import { CORE_MANAGED_FILES } from './core-managed-files.mjs';
 ...
 * does not yet track (C64b-3 / C64b-8).
 *
 ...
 * the result (the C64b-3 report-only advisory, the C64b-8 core-doc WARN gate,
 * or `--apply-new` adoption).
 *
 * @param {object} params
 ...
 * @returns {Promise<{ newFiles: string[], coreMissing: string[] }>}
 *   `newFiles` — all detected new managed files (canonical, sorted).
 *   `coreMissing` — required core managed docs (C64b-7) absent from
 *   `managed.files` (canonical, sorted). Independent of disk presence.
 */
export async function detectNewManagedFiles({ config, harnessRepoPath }) {
  ...
  const coreMissing = CORE_MANAGED_FILES
    .map(f => canonicalizeTargetPath(f))
    .filter(f => !declared.has(f))
    .sort();

  return { newFiles, coreMissing };
}
 ...
 * @param {boolean} [args.quiet=false] - CS64b (C64b-8): when true, suppress the
 *   new-managed-file advisory and the core-doc WARN (success/advisory output
 *   only). Hard errors still surface. Default false preserves  output.
...
export async function sync(args) {
  ...
  // ── CS64b new-managed-file reconciliation (C64b-3 / C64b-8) ──────────────
  ...
  const { newFiles: detectedNewFiles, coreMissing: detectedCoreMissing } =
    await detectNewManagedFiles({ config, harnessRepoPath });

  const coreSet = new Set(CORE_MANAGED_FILES.map(f => canonicalizeTargetPath(f)));
  // C64b-3 advisory is report-only and covers NON-core new files; the required
  // core docs are surfaced by the stronger C64b-8 WARN below instead.
  const advisoryCandidates = detectedNewFiles.filter(f => !coreSet.has(f));
  ...
  // Outstanding (un-adopted) sets drive the result fields and the WARN/advisory.
  const adoptedSet = new Set(adoptedNewFiles);
  const newManagedFiles = advisoryCandidates.filter(f => !adoptedSet.has(f));
  const coreMissing = detectedCoreMissing.filter(f => !adoptedSet.has(f));

  // C64b-8: WARN (membership-based, warn-only this minor) for every required
  // core managed doc still absent from managed.files. Does NOT change
  // driftDetected or the exit code. C64b-3 advisory (report-only) lists the
  // remaining non-core new files. Both are suppressed under --quiet
  // (success/advisory output only; hard errors still surface).
  if (!quiet) {
    for (const missing of coreMissing) {
      warnings.push(
        `Required core managed doc "${missing}" is not in managed.files. ` +
        `Run 'harness sync --apply-new' to adopt it ` +
        `(warn-only this release; escalates to an error in a future major).`
      );
    }
    if (newManagedFiles.length > 0) {
      warnings.push(
        `New managed files are available but not tracked in managed.files: ` +
        `${newManagedFiles.join(', ')}. Run 'harness sync --apply-new' to adopt them ` +
  ...
  if (mode === 'apply') {
    ...
    // (idempotency). Re-parse the original raw config to preserve its on-disk
    // structure — only managed.files is mutated; all other fields/formatting
