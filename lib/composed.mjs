 ...
 * 2. Duplicate mapping entries for the same region → `EMERGE_LEGACY_BAD_MAPPING`
 * 3. Actual region not covered by any mapping entry → `EMERGE_LEGACY_UNMAPPED`
 * 4. Mapping entry content that does not correspond to any actual region →
 *    `EMERGE_LEGACY_BAD_MAPPING`
 * 5. `map_to_block` target block ID absent from template or allowedBlockIds →
 *    `EMERGE_LEGACY_BAD_MAPPING`
 *
...
function validateLegacyMapping(
  ...
  // Step 2: detect duplicate mapping entries.
  const seen = new Set();
  for (const r of entries) {
    const c = normalizeLF(r.content ?? '');
    if (seen.has(c)) {
      throw new ComposedMergeError(
        `Duplicate legacy mapping entry for content: "${c.slice(0, 60)}"`,
        'EMERGE_LEGACY_BAD_MAPPING'
      );
    }
    seen.add(c);
  ...
  // Step 3: every actual region must be covered by exactly one mapping entry.
  for (const region of actualRegions) {
    const covered = entries.some(r => normalizeLF(r.content ?? '') === region);
    if (!covered) {
      throw new ComposedMergeError(
        `Consumer file contains legacy content outside local blocks that is not ` +
        `covered by any legacyMapping entry: "${region.slice(0, 60)}"`,
        'EMERGE_LEGACY_UNMAPPED'
      );
    }
  }

  // Step 4: every mapping entry must correspond to an actual region.
  for (const r of entries) {
    const c = normalizeLF(r.content ?? '');
    if (!actualRegions.includes(c)) {
      throw new ComposedMergeError(
  ...
  // Step 5: validate map_to_block targets.
  for (const r of entries) {
    if (r.action === 'map_to_block') {
      if (!r.block_id || !templateParsed.blocks.has(r.block_id)) {
        throw new ComposedMergeError(
